...
唯一窄例外精确来自方法机制文档 `CONTENT_ROUTING_REFERENCE_REGISTRY_MACHINE_CONTRACT["schema_freeze_exception_fields"]` 的26项集合，只允许在 content-routing reference registry 及其 materialization/qualification 机器协议冻结时预登记。该集合必须由专用 constraint 从同一个权威 AST literal 解析，逐项具有 `target_required` 生命周期和 `allowed_in_claims=false`；登记不表示 validator、loader、producer、writer、consumer、runtime、qualification、promotion 或正式 artifact 已实现，也不得提升 `not_materialized`、`document_ecosystem_synchronized`、`cpu_conformant` 或后续状态。`configs/content_routing_reference_registry.json` 不存在且正式实现尚未闭合期间，candidate 固定 `supports_paper_claim=false`。不得从 `governance`、`schema`、字段类别、命名或自然语言推导其他预登记例外，不得在字段生命周期 JSON 中维护第二个 exception list。除该精确26项外，runtime/record 字段与 writer、consumer、aggregator 同一原子迁移的门禁原样有效；未来首次实现该 artifact 时，exact validator 与唯一 loader 必须在同一实现原子闭合。
...
| content_routing_reference_materialization_manifest_digest | provenance | none | true | false | false | `content_reference_materialization_manifest_semantic_identity_v1`：非主张 reference candidate 的 generation、runtime、raw member 与 registry 身份单向汇总语义摘要；不绑定 qualification report。 |
| content_routing_reference_materialization_manifest_file_sha256 | provenance | none | true | false | false | `content_reference_materialization_manifest_exact_file_bytes_v1`：materialization manifest 规范 UTF-8 单 LF 文件的精确字节 SHA-256，由 qualification 与 promotion 外层记录消费。 |
| content_routing_reference_qualification_report_digest | provenance | none | true | false | false | `content_reference_qualification_report_semantic_identity_v1`：真实 CUDA qualification 对 manifest、registry、raw member 和15项检查的规范语义摘要。 |
| content_routing_reference_qualification_report_file_sha256 | provenance | none | true | false | false | `content_reference_qualification_report_exact_file_bytes_v1`：qualification report 规范 UTF-8 单 LF 文件的精确字节 SHA-256，由 promotion 外层记录消费。 |
| content_routing_reference_qualification_ready | governance | none | true | false | false | `content_reference_qualification_all_checks_ready_v1`：仅当15项真实 qualification 检查全部为 `pass`、无 `blocked`、无 error/skip 且未使用 fixture/proxy 时为 true。 |
