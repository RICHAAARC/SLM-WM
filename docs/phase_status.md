...
- 当前方法语义层级: `partially_cpu_verified`。权威不变量及追踪关系已经冻结；风险信号、风险硬包络、LF/tail、Q/K 单调步长、Null Space、实际 dtype 合成和结果级科学内容重建具有独立 CPU 性质测试。其余核心不变量仍须完成独立正反例审计, 因而当前不能提升为完整 CPU 方法验证。
...
| 能力 | 代码状态 | 正式 GPU 证据状态 |
| --- | --- | --- |
| 核心方法算子 | 风险、载体、Q/K、Null Space、唯一合成与磁盘级科学内容重建已有真实实现和独立 CPU 性质测试；仍需完成其余不变量的独立方法审计 | 方法审计通过后执行 Colab GPU 科学预检 |
