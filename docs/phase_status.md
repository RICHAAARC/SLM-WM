...
- 当前方法语义层级: `specified`。权威不变量及追踪关系已经冻结, 但跨模块 CPU 性质尚未全部达到 `cpu_verified`。
...
1. **分支风险场**: 权威方法要求分支风险同时约束支持、方向和实际 dtype 写回幅值。当前代码已经把预算用于 Null Space 支持, 但风险硬包络与解析范围标定仍属于本构建单元的阻断工作。
...
4. **高斯幅值尾部截断载体**: 权威方法只按高斯幅值分布的尾部集合选择鲁棒补充分支, 不把该分支解释为空间高频。当前模板的仅 L2 归一化性质尚未通过独立 CPU 门禁。
...
8. **科学内容身份**: 权威身份协议要求版本化 Tensor 摘要联合绑定风险、预算、资格 mask、Jacobian 候选/响应/基底、三分支更新、量化写回和逐层 Q/K 原子。当前联合摘要的运行生产与正式门禁消费尚未闭合。
9. **构造式更新协议**: 权威方法要求 LF 与尾部模板分别执行安全投影和风险预算硬包络缩放, 注意力分支从风险允许的最大步长执行单调回溯, 三分支在核心方法算子中合成一次并复验实际 dtype 写回。当前风险合成算子尚未闭合；项目不声明求解联合标量 `argmax`。
...
| 能力 | 代码状态 | 正式 GPU 证据状态 |
| --- | --- | --- |
| 核心方法算子 | 权威不变量已冻结；局部算子大部分存在, 跨模块风险写回和逐列公式尚未完成 CPU 语义门禁 | 需要核心语义完成后执行 Colab GPU 科学预检 |
...
5. 方法完成审计形成受治理报告后, 才进入 `experiment_protocol_validation` 处理9重复、fixed-FPR、baseline 公平性和论文结果协议。
