...
本文档记录当前分阶段构建推进状态。它只描述阶段门禁、输入、输出、阻断项和当前可继续推进边界, 不承载正式论文实验结论。

本文件已于 2026-07-09 依据 `5bcbe8f:docs/phase_status.md` 这一最后可读中文版本进行重建, 并结合当前仓库结构更新到现行三层运行架构。历史乱码版本中的不可读内容不再作为事实来源; 若需要追溯某一旧条目的完整描述, 应以 Git 历史中最后可读版本和对应提交产物为准。

## 当前运行层级口径

`diagnostic / smoke` 是一个非论文运行类别, 不是两个正式运行配置层级。它只用于环境、依赖、模型加载、Drive 挂载和最小机制预检, 不参与正式统计协议, 也不拥有独立 prompt 配置文件。
...
| 构建单元 | 当前状态 | 当前结论 |
| --- | --- | --- |
| `stage_00_core_package_boundary_freeze` | completed | `main/` 核心包边界已冻结, 核心方法层不得反向依赖 Notebook、Drive、外部 baseline 或 harness。 |
| `stage_01_algorithm_primitives` | completed | 纯算法原语已实现并通过轻量测试。 |
| `stage_02_core_method_smoke_test` | completed | 合成 latent smoke 已覆盖 key 区分、rescue 边界和 attestation 分层。 |
| `stage_03_sd3_runtime_adapter` | completed | SD3 / SD3.5 runtime adapter 与 fallback 记录链路已建立。 |
| `stage_04_minimal_diffusion_latent_injection` | completed | 真实 SD3.5 最小 latent injection 与运行时预检入口已合并到诊断预检路径; 该路径不支持正式论文统计主张。 |
...
| `stage_12_threshold_calibration_metrics` | completed | fixed-FPR 阈值校准、clean negative 门禁和质量指标摘要已建立。 |
| `stage_13_attack_matrix_regeneration` | completed | 再扩散攻击闭环与常规/几何攻击闭环均有正式 workflow, 需以实际运行包判断当前运行层级是否闭合。 |
| `stage_14_external_baseline_comparison` | completed | 主表 external baseline 采用 method-faithful SD3.5 adapter 与 official reference 双链路, 通过受治理导入协议进入共同对比表。 |
| `stage_15_internal_ablation_evidence` | completed | 内部消融证据链路已建立, 结果必须由 records 重建表格和报告。 |
| `stage_16_paper_artifact_evidence_audit` | completed | 论文证据审计、图表重建和缺口报告能力已建立。 |
| `stage_17_pilot_full_submission_freeze` | in_progress | 投稿冻结门禁已建立; 是否满足投稿级结论取决于当前运行层级的完整结果包、样本规模、baseline 和攻击矩阵证据是否全部闭合。 |
...
1. Notebook 统一位于 `paper_workflow/notebooks/`, 只负责挂载 Drive、拉取仓库、选择运行层级和调用 repository modules。
2. 正式逻辑位于 `main/`、`experiments/`、`paper_experiments/` 或 `scripts/`, 不应写回 Notebook cell。
3. 论文结果闭合入口为 `paper_workflow/notebooks/paper_result_closure_run.ipynb`, 该入口支持 `probe_paper`、`pilot_paper` 与 `full_paper`。
4. GPU 服务器可使用 `scripts/run_gpu_server_workflow.py` 和 `scripts/run_gpu_server_result_closure.py` 直接运行主方法、external baseline、official reference 与结果闭合流程; 该路径不依赖 Notebook。

## 当前证据闭合判断规则

| 判断项 | 可认为通过的条件 |
| --- | --- |
| 主方法机制闭合 | attention geometry、attention latent injection、aligned rescoring、threshold calibration 均来自同一运行层级且 manifest / digest 可追溯。 |
| 攻击矩阵闭合 | real attack evaluation 与 conventional geometric attack evaluation 均完成, formal attack detection records 已进入 attack matrix。 |
...
1. `pilot_paper` 只能支持 FPR=0.01 与600个 prompt 的运行层级结论, 不能自动提升为 `full_paper` FPR=0.001 结论。
2. `full_paper` 仍需要一次完整运行来确认样本规模、低 FPR 阈值、外部 baseline、攻击矩阵和图表结果全部闭合。
3. 若某个 GPU workflow 结果包失败, 应优先检查对应 environment report、manifest、formal records 和 package input manifest, 不应手工补表。
4. 当前 `docs/phase_status.md` 只作为推进状态说明, 论文主张仍必须以 records、tables、figures、reports 和 manifests 为事实来源。
...
| command | result |
| --- | --- |
| `pytest -q` | pass, 321 passed |
| `python tools/harness/run_all_audits.py` | pass, 8/8 audits passed |

## 历史阶段明细基线

以下明细来自 `5bcbe8f:docs/phase_status.md` 最后可读中文基线, 用于保留分阶段推进的可读历史记录。本文已对旧非论文 probe prompt 文件名做最小同步, 以避免和当前 `probe_paper` 运行层级混淆。若历史明细与上方现行三层运行架构、Notebook 目录或服务器入口描述存在冲突, 以上方“当前运行层级口径”和“当前总体状态”为准。
...
| executor | `codex_agent` |
| execution_date | `2026-06-20` |
| input_manifest | `outputs/colab_drive_workflow-20260620T114217Z-3-001.zip`; `outputs/prompts.zip`; `configs/paper_main_probe_paper_prompts.txt`; `configs/paper_main_pilot_prompts.txt`; `configs/paper_main_full_prompts.txt` |
| expected_output_manifest | `outputs/prompt_event_protocol/manifest.local.json` |
| expected_outputs | `configs/paper_main_probe_paper_prompts.txt`; `configs/paper_main_pilot_prompts.txt`; `configs/paper_main_full_prompts.txt`; `outputs/prompt_event_protocol/prompt_records.jsonl`; `outputs/prompt_event_protocol/event_records.jsonl`; `outputs/prompt_event_protocol/prompt_manifest.json`; `outputs/prompt_event_protocol/split_manifest.json`; `outputs/prompt_event_protocol/event_protocol_manifest.json`; `outputs/prompt_event_protocol/prompt_statistics.json`; `outputs/prompt_event_protocol/manifest.local.json` |
| blocking_items | 无。 |
...
3. `configs/paper_main_probe_paper_prompts.txt`、`configs/paper_main_pilot_prompts.txt` 和 `configs/paper_main_full_prompts.txt` 当前分别包含 10、600 和 6000 条 prompt。
