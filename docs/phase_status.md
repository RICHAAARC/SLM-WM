...
| 结果物化与审计 | 已实现单 repeat 的7类随机化包双来源身份核验、自包含原字节封装、精确9组件加3个跨 repeat 不变包的版本化聚合来源构造与独立复验；同一聚合来源可独立重建45个 method-repeat 阈值、全样本与质量匹配配对优势、正式消融必要性和精确9重复 FID/KID。component、aggregate provenance 与单项测量结果均不自动支持论文 claim | 尚需由真实 GPU 结果物化9个组件, 并补齐绝对检测、逐攻击、wrong-key、失败案例和最终论文结论桥接 |
