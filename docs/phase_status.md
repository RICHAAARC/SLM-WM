...
- 当前方法语义层级: `cpu_verified`。权威不变量、追踪关系、核心实现、真实模型绑定和科学内容重建已经完成独立正反例审计；正式完整方法路径未发现阻断级方法偏离、代理、占位或静默替代。审计证据见 `docs/builds/method_conformance_report.md`。
...
三类运行层级执行同一方法实现、攻击矩阵、baseline 流程、消融流程、检测器、fixed-FPR 统计实现和结果闭合命令。三级目标 FPR 分别为0.1、0.01和0.001；Prompt 数量与固定划分样本数同时递增, 以支撑更严格工作点和更高统计强度。
...
- 共同协议记录拒绝任何非正式证据标记。
- probe、pilot 和 full 使用同构闭合判断, 并分别在 FPR=0.1、FPR=0.01和 FPR=0.001 工作点形成对应层级结论。

## 当前状态表

| 能力 | 代码状态 | 正式 GPU 证据状态 |
| --- | --- | --- |
| 核心方法算子 | 风险、载体、Q/K、Null Space、唯一合成与磁盘级科学内容重建已有真实实现, 独立方法审计未发现阻断级方法偏离或弱实现 | 执行 Colab GPU 科学预检 |
...
| 数据集质量 | 已实现单 repeat 正式特征导入与精确9重复原始 Inception 特征联合重建；FID、KID mean 与 KID std 不读取或平均单 repeat 派生表 | 需要9个注册重复的真实 GPU Inception 特征结果 |
| 外部 baseline | 已实现方法忠实主表与官方复现双链 | 需要完整 baseline 结果包 |
| 结果物化与审计 | 已实现单 repeat 的8类随机化包双来源身份核验、自包含原字节封装、精确9组件加3个跨 repeat 不变包的版本化聚合来源构造与独立复验；同一聚合来源可独立重建45个 method-repeat 阈值、全样本与质量匹配配对优势、正式消融必要性、单模型内部风险参数敏感性和精确9重复 FID/KID。component、aggregate provenance 与单项测量结果均不自动支持论文 claim | 尚需由真实 GPU 结果物化9个组件, 并补齐绝对检测、逐攻击、wrong-key、失败案例和最终论文结论桥接 |
