...
三类运行层级执行同一方法实现、攻击矩阵、baseline 流程、消融流程、检测器、FPR=0.1 工作点和结果闭合命令。差异仅限 Prompt 数量、固定划分后的样本数量与由样本规模带来的统计强度。
...
| `pilot_paper` | 700 | 30 | 330 | 340 | 0.1 | `pilot_claim` |
| `full_paper` | 7000 | 300 | 3300 | 3400 | 0.1 | `full_claim` |
...
- 共同协议记录拒绝任何非正式证据标记。
- probe、pilot 和 full 使用相同闭合判断与 FPR=0.1 工作点; 统计置信强度仅由各自样本规模决定。
