...
- 当前方法语义层级: `partially_cpu_verified`。权威不变量及追踪关系已经冻结；风险信号、风险硬包络、LF/tail、Q/K 单调步长、Null Space 和实际 dtype 合成具有独立 CPU 性质测试, 结果级联合证据与正式门禁仍未闭合。
...
8. **科学内容身份**: 权威身份协议要求版本化 Tensor 摘要联合绑定风险、预算、资格 mask、Jacobian 候选/响应/基底、风险清理后逐分支 JVP、三分支更新、量化写回和逐层 Q/K 原子。量化合成子链已经绑定原始与候选 latent、活动分支、包络、dtype/shape 和共同回溯轨迹；跨全部科学角色的总联合摘要仍在后续构建单元闭合。
...
| 能力 | 代码状态 | 正式 GPU 证据状态 |
| --- | --- | --- |
| 核心方法算子 | 风险、载体、Q/K、Null Space 与唯一合成已有真实实现和独立 CPU 性质测试；结果级联合证据及正式消费门禁待闭合 | 需要结果级门禁完成后执行 Colab GPU 科学预检 |
