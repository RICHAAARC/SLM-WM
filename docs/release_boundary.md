...
`scripts/write_pilot_paper_complete_result_package.py` 归档当前论文运行层级的受治理输出、Prompt 定义、依赖锁和证据重建代码。共享代码白名单只允许 `scripts/`、`paper_experiments/`、`experiments/`、`main/`、`configs/` 与必要方法文档, 不包含 `paper_workflow/`。因此该结果包可以在 CPU 服务器脱离 Notebook、Colab session 和 Drive helper 复核科学执行绑定与18步证据闭合。
