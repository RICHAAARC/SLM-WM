...
后续方法一致性审计必须把以下两项视为当前 SLM-WM 的冻结设计定义, 而不是待补齐能力或弱实现迹象：

1. 完整716维特征算子由512维归一化 CLIP `image_embeds` 与204维显式手工结构描述符连接而成。204维分量只表示本文档冻结的颜色、梯度和空间池化结构坐标, 不表示通用感知质量模型、学习式感知 embedding 或独立语义模型。它作为完整 Jacobian 的正式结构约束坐标真实参与 JVP/VJP, 因而“手工”只描述坐标构成, 不表示 proxy、placeholder 或可省略的诊断量。
2. “潜流形”只表示当前 latent 点处716维特征水平集的一阶局部切空间解释。正式数值对象是经分支风险支持和残差门控得到的 Jacobian 低响应子空间；方法不声明学习或构造全局流形、坐标图、测地线、曲率、回缩或全局拓扑。缺少这些全局对象不构成当前方法实现缺口。

只有当实现没有真实计算上述204维坐标、没有让完整716维算子进入 Jacobian, 或论文把局部切空间越界表述为全局流形学习时, 才应登记为方法定义与实现不一致。最终图像感知质量必须由实验层的 FID/KID、配对质量指标或人工评价独立证明, 不得由204维结构保持门禁替代。
...
| `qr_reference_solve_protocol` | `right_upper_triangular_solve_without_explicit_inverse` |
...
这里 `StableJSON` 使用 `ensure_ascii=false`、键名升序和无空白分隔符 `(',', ':')`；摘要不含 YAML 排版、配置文件绝对路径或派生摘要本身。运行时实际 pipeline、VAE、Transformer、scheduler 的完全限定类名，VAE `scaling_factor`/`shift_factor`、latent/vision dtype、冻结 attention 层名、`attention_operator_schedule_index=7` 与 `public_detection_schedule_index=7` 都必须逐项等于上述配置, 并与 `formal_method_config_digest` 一起进入原子证据。注入回调索引6、10、14只决定当前写回发生在哪个 post-step latent；所有内容 Jacobian、风险场 Q/K、注意力梯度、回溯和写回复验统一使用 `scheduler.timesteps[7]` 作为冻结 Q/K 科学算子 timestep。禁止把 `step_index+1` 对应的 post-step timestep 当成注意力算子 timestep。VAE 编码和解码只能直接读取实际 `vae.config.scaling_factor` 与 `vae.config.shift_factor`; 任一字段缺失时必须失败, 不允许用1或0补齐。任一 revision 缺失、snapshot 漂移、类身份改变、VAE 参数改变或缺失、层名缺失、dtype 漂移、两个冻结索引不等于7、两者身份不一致或配置摘要不一致时, 当前科学单元必须失败。模型族名称或 `main` 分支名不能替代精确 revision。
...
$F_{\mathrm{CLIP}}$ 是冻结 CLIP 的512维投影后 `image_embeds` 的完整归一化 image embedding。正式特征算子的前向顺序精确为：latent 先除以 VAE `scaling_factor` 再加 `shift_factor`, VAE 解码结果执行 `decoded / 2 + 1 / 2` 并截断到 $[0,1]$；随后以 bicubic 缩放到224x224, 固定 `align_corners=false`、`antialias=true`, 再按通道均值 $(0.48145466,0.4578275,0.40821073)$ 与标准差 $(0.26862954,0.26130258,0.27577711)$ 归一化后送入冻结 CLIP。投影特征来源只能是 `image_embeds`, 并沿最后一维执行 L2 归一化；不允许用投影前 `pooler_output` 或任一隐藏状态替代。风险信号使用同一次 `last_hidden_state`, 其中索引0是 CLS, patch token 从索引1开始。运行层必须消费 `main/methods/semantic/feature_protocol.py` 的同一纯数据协议及其摘要, 不得在外层重新声明图像尺寸、插值、通道归一化、token 索引或特征来源。

204维结构向量严格按以下坐标顺序拼接：3维 RGB 通道均值、3维 RGB 总体标准差、3维水平绝对梯度均值、3维垂直绝对梯度均值, 以及按 channel-row-column 顺序展平的192维 RGB 8x8 自适应平均池化。联合向量严格先放512维归一化投影 CLIP embedding, 再放204维结构坐标。正式算子只接受单样本 batch；任一分量非有限、CLIP 投影能量精确为零、宽度漂移或协议摘要不一致都必须失败。正式输出宽度固定为716, 不允许坐标选择、随机投影、低维草图、零向量归一化替代或把204维向量宣称为一般感知质量模型。
...
不得使用跨列共享 RMS。PSD-CG 最大64次、阻尼为0；只有右端项精确为零时才允许零步返回。候选收敛和函数返回前都必须用最终解重新执行线性算子, 以 $r_{true}=b-Ax$ 重算残差, 并唯一要求 $\|r_{true}\|_2/\|b\|_2\le10^{-6}$。绝对残差阈值和只依赖递推 residual 的收敛判定均不得替代该门禁。其余门禁为 $e_i\ge0.01$、$r_{b,j}^{\mathrm{QR}}\le10^{-4}$、$\|N_b^\top N_b-I\|_F\le10^{-5}$。
...
`keyed_prg_version` 必须等于 `sha256_counter_normal_icdf_table20_float32`。上述对象通过 UTF-8、`ensure_ascii=false`、键名升序和无空白分隔符 `(',', ':')` 形成 stable JSON, 再执行 SHA-256 得到32字节 domain digest。计数器从0开始, 以16字节无符号大端编码拼接在 domain digest 后；每个计数器输入再执行 SHA-256。连续32字节输出块组成 MSB-first 大端比特流。Gaussian 路径跨块连续提取20位索引 $i$, 不按字节边界丢弃剩余比特, 并查询
...
尾部载体协议固定为 `slm_wm_tail_robust_carrier_protocol`, 正文同时绑定 `tail_fraction`、绝对幅值降序且展平索引升序的选择规则和密钥 PRG 版本。仅图像检测记录必须保存模板 NCHW 形状、元素总数、$\lceil n\gamma\rceil$ 选中数、阈值、实际保留比例及模板内容摘要；raw 与 aligned 路径必须具有相同 latent 形状、LF 模板摘要、尾部模板摘要、尾部阈值和保留比例。
...
$W_i=0$ 或任一有限的加权中心化能量不大于 $\epsilon_{\mathrm{num}}^2$ 时该行无效。每个 batch 先对有效 row 取均值；没有有效 row 时该 batch 分量分数为0。该0分规则只适用于全部输入有限的数学退化行；NaN 或 Inf 必须在进入相关计算前失败。随后依次对 batch 和冻结层等权平均：
...
重算真实四分量梯度 $g_A=\nabla_zs_A(z)|_{z=z_t^{\mathrm{base}}}$。注入回调在索引6、10、14执行, 对应的 post-step 状态索引分别为7、11、15；这三个状态只表示被修改的 latent。所有 Q/K 关系、attention stability、几何梯度、候选评分与实际写回评分必须统一使用固定的 `attention_operator_schedule_index=7`, 即 `scheduler.timesteps[7]`。每条更新原子同时记录 callback step、scheduler callback timestep、post-step 索引与 timestep、固定算子索引与 timestep, 并要求固定算子索引不随 callback 改变。令
...
两个 Corr 都要求观测量和模板分别具有有限、非零的中心化能量。实现必须先分别归一化两个方向再计算内积, 以保持对可表示非零尺度的尺度不变性；常量 Tensor、非有限 Tensor 或零能量模板必须立即失败, 不得被改写成合法0分。

仅图像检测密钥计划固定包含 `registered_watermark_key` 与 `registered_wrong_key_negative` 两个角色。wrong-key 由 `registered_key_and_sha256_domain_separated_wrong_key` 从当前注册水印密钥确定性派生；记录只保存两个材料摘要、版本化计划正文和计划摘要, 不保存密钥原文。完整注入与 carrier-only 更新中的 `watermark_key_material_digest_random` 必须等于计划中的注册密钥摘要；注册密钥检测模板必须与嵌入模板相同, wrong-key 模板必须在每个分支内部唯一且与注册密钥模板不同。只修改 `sample_role` 不能建立 wrong-key 证据。
...
其中 `public_detection_noise_domain=public_image_only_qk_detection_noise`, $D_{det}$ 是 `versioned_key_prg_reconstruction` 中逐字段冻结的公开 domain。该调用完整执行 SHA-256 大端计数器、MSB-first 连续20位索引提取和冻结 Q20 中点逆 CDF float32 表查询, 在 CPU 完成实际 dtype 转换后再搬运到 $\hat z$ 的设备。加噪 latent 必须由冻结 scheduler 的真实

$$
z_{det}=\operatorname{scheduler.scale\_noise}(\hat z,t_{det},\epsilon_{det})
$$

得到；缺失 `scale_noise` 时失败, 不允许线性 latent/noise 混合替代。Transformer 条件固定为 `public_detection_conditioning_protocol=sd3_empty_text_triplet_without_cfg` 和 `public_detection_condition_text=""`。该噪声与水印密钥、Prompt、生成 seed 和生成轨迹无关。

检测器不得读取 Prompt、源 latent、采样轨迹、样本级 Null Space、生成端 Q/K 原子或生成 seed。raw 图与对齐图必须分别重新编码并重新提取 Q/K, 原子角色精确为 `raw_detection_image` 和 `aligned_detection_image`。`image_alignment_enabled=true` 必须同时满足 `attention_geometry_enabled=true`; 单独开启图像配准没有可定义的注意力目标, 必须在加载模型前拒绝。measurement 中 alignment 与 aligned 内容分数实行双向存在约束：alignment 存在当且仅当 `aligned_content_score` 存在, 且 aligned LF/tail 分支分数必须同时完整；不允许保存无 aligned 内容重编码的 alignment, 也不允许保存没有 alignment 身份的 aligned 分数。
...
在抽样 token 索引范围内确定性均匀选择。所有锚点和残差都使用 `normalized_xy_token_centers_corner_endpoints` 坐标, 角点 token 中心分别为 -1 与1；关系图采样和图像仿射重采样统一使用 `align_corners=true`。对候选恢复变换, 仅具有有效双线性覆盖的锚点进入分母集合 $\mathcal V$。锚点同时满足观测匹配唯一且归一化 xy 欧氏残差 $d_j\le0.20$ 时记为内点, 因而
...
无有效覆盖锚点时 $r_{\mathrm{inlier}}=0$；无内点时平均内点残差为非有限失败值。`attention_anchor_count=12`、`attention_residual_threshold=0.20` 和 `attention_minimum_inlier_ratio=0.50` 是正式方法的预注册结构常量。完整 measurement 配置只保存在顶层运行 manifest；alignment 和样本记录保存连续测量原子及 measurement 配置摘要, calibration/test 必须绑定同一摘要。calibration 或 test 数据不得选择、放宽或替换这三项常量；alignment 存在时, `raw_attention_geometry_score`、`attention_geometry_score`、`registration_confidence`、`attention_sync_score` 和 `aligned_content_score` 五个连续原子必须同时完整且有限。alignment 与 aligned 内容分数还必须满足双向存在约束。任一测量原子缺失、数值异常、数值漂移或配置摘要不一致都使当前记录失败, 不得因任一几何原子异常而静默改用 raw-only 判定。
...
`slm_wm_tensor_content` 按协议版本、dtype、shape 和连续原始字节绑定 Tensor 身份。最终图像不能只绑定 PNG 文件字节, 因为相同像素可以使用不同压缩参数编码。`slm_wm_image_rgb_uint8_content` 必须先把持久化图像解码为 RGB, 再按以下固定顺序计算规范 RGB uint8 像素摘要:
