...
- `content_survival_observation_colab_20260723T191802Z_1468eac` 的1 Prompt A100结果中，full与carrier-only两套HF均为33-key rank-1且preservation通过，但full相对carrier的最终图像Q/K gain仅约`2.06e-5`，未达到固定`1e-4`门，因而没有正式runtime artifact。当前CPU候选保持step-18 late-HF、预算、scorer和阈值不变，只把full专属late-Q/K方向与候选接受改为registered-only、carrier-frozen的同构suffix：隔离重放剩余一步生成，再经真实decode、VAE重编码、公开噪声、scheduler索引7和冻结Q/K层评分；只有blind与carrier-frozen pair两条gain同时达到固定门才写回，否则保留late-HF并失败关闭。该候选尚未经过新的GPU验证。
