...
4. 将同一个不可变聚合来源对象传给 `paper_experiments.runners.randomization_paired_superiority`。该入口逐 repeat 使用对应5个阈值构造主方法与4个 baseline 的真实攻击后配对 outcome, 并核验仅图像盲检、真实攻击图像、攻击 seed、生成 seed、密钥和基础 latent 身份。对每个 Prompt 先在 repeat 内平均完整攻击集合, 再平均9个注册 repeat；bootstrap 和 Hoeffding 推断的独立单位仍是34 / 340 / 3400个 test Prompt, 不得把9重复展开为9倍独立样本。4个主表 baseline 构成唯一总体优势 Holm 家族。
5. 将同一个不可变聚合来源对象传给 `paper_experiments.runners.randomization_ablation_necessity`, 从9份真实重运行消融原始记录重建机制必要性统计；不得读取或平均单重复派生表。
6. 构造 baseline 正式候选并执行受治理导入, 同时传播唯一 `randomization_aggregate_digest`。
7. 写出主方法与 baseline 的共同协议结果记录, 重建 fixed-FPR 表、优势表与真实失败案例图。
8. 执行证据审计、投稿就绪审计、结果闭合语义门禁和完整结果打包。
