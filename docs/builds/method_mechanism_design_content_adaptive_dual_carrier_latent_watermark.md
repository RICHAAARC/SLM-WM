...
4. `configs/content_routing_reference_registry.json` 精确绑定隔离参数划分、qualification 复验身份、精确文件身份和冻结 `g_ref/r_ref/q_ref`；单 Prompt 资格化复验 registry 与方法身份摘要，不在该 Prompt 上重算 reference 标量。
...
- 三个 reference 标量统一使用精确有理数 `19/20` 的 nearest-rank 分位数，索引规则为 `(19*n+19)//20-1`；总体分别是映射前全部有限正 `G`、latent 分辨率全部有限正 `d_R`、映射前全部有限正 `d_Q` 的跨成员拼接，不执行逐图像汇总。
...
内容路由 reference 的算法语义只保留以下最小边界；exact machine schema 与 validator 的唯一所有者是 `experiments/protocol/content_routing_reference_registry.py`。本文件不得复制 exact key map、predicate map 或可执行 AST schema，registry payload producer 与 fixed-path loader 必须消费 owning module 中同一个 schema 对象。

唯一 registry 保存与 calibration/test 均隔离的有序方法参数划分身份、按成员顺序计算的 Prompt/seed 摘要、真实 `B=1` 样本数、精确 SD3.5 model/revision、formal/dependency/runtime-preprocess 身份、固定三 population、三个 reference 标量和 registry semantic digest。三个 population 顺序固定为映射前 `G`、latent 分辨率 `d_R`、映射前 `d_Q`；每个 population 保存严格正值数量、有序 member-record 摘要和按 member 顺序、member 内 flat-NCHW 顺序过滤严格正值后且排序前形成的 CPU float32 一维 Tensor 内容摘要。member-record 摘要绑定 population kind、连续零基 sequence index、真实 generation input identity digest 和 raw observation Tensor 内容摘要。

分位算法 token 精确固定为 `nearest_rank_full_sort_exact_rational_19_over_20_v1`：拼接完整严格正 float32 总体，完整升序排序，以 `rank=(19*n+19)//20`、`index=rank-1` 选择 binary32 值。禁止逐图像统计、浮点 `0.95*n`、线性插值、`quantile`、`kth` 或 `topk` 替代。三个 JSON reference 标量必须是 exact float、有限、严格正，且经 `struct.unpack(">f", struct.pack(">f", value))[0]` 后逐值不变；任意 JSON double、整数或 bool 均失败关闭。

真实 GPU producer 必须从同一次运行的实际 Prompt、generation seed、canonical base-latent identity、精确 model/revision 与冻结生成配置构造每成员 generation identity。CPU fixture 只验证 payload 结构和聚合公式，不得被描述为 real、verified 或 qualified；真实 qualification 必须独立重建 generation identity、三个 raw population、完整 P95 和 registry identities。

`content_routing_reference_registry_digest` 是 registry 顶层对象删除且仅删除自身字段后的规范语义摘要。`content_routing_reference_registry_file_sha256` 是精确候选文件字节的独立身份，不写入 registry 自身。真实 qualification 必须从 actual candidate registry bytes 同时复算 semantic digest 与 exact file SHA，并逐项核对真实 Prompt/seed/runtime/raw population/P95。

promotion 只能把 qualification 已通过的同一 candidate registry 原始 bytes 逐字节写到固定路径 `configs/content_routing_reference_registry.json`；禁止重算、重序列化、重排字段或以 fixture/default/synthetic 替代。promotion 后必须重新读取固定文件并复核 semantic digest 与 exact file SHA 均等于 qualification 输入。loader 不接受路径、默认值或 fallback，仍要求调用方同时提供 expected semantic digest 与 expected file SHA，并按 file SHA、strict JSON、semantic digest、内部一致性的顺序在任何模型前向前失败关闭。
...
本阶段不实现 writer、manifest、qualification、promotion 或 runtime 接线，不生成 candidate、registry、output 或 config artifact。
### 10.3 内容载体
