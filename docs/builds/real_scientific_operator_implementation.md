...
7. 所有样本只从最终图像重新编码并检测；
8. calibration registered-key clean negative 先生成不含阈值和判定字段的原始测量, 再按版本化 Prompt 散列确定性拆分为互斥的1/3 `window_fit` 与2/3 `threshold_freeze`；前者拟合几何门和最宽可行 rescue 窗口, 后者用判定等价连续分数冻结唯一最终 fixed-FPR 阈值；
