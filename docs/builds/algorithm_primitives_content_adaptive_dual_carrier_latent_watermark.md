...
10. 对正式名义 replay，在剩余 scheduler 完成后的 pre-VAE latent 上追加一次固定能量、HF-tail-only 的 registered-key 终端载体；该写入不进入 scheduler callback，也不改变 Q/K 几何链。
11. 从待检图像独立提取 HF-tail 正式归属分数和真实 Q/K 几何关系；LF/dual 分数只保留为机制诊断与消融。
12. 对近阈值失败样本执行几何恢复、图像回正、重新编码和同阈值内容重判。
13. 使用完整决策器在独立 calibration split 上校准 fixed-FPR。

以下内容不属于目标方法：

- 716维特征水平集及其一阶局部切空间解释。
- Jacobian Null Space、完整 JVP/VJP 和 PSD-CG。
- 多 scheduler 时刻重复写回；正式终端载体只允许位于 scheduler 完成后、最终 VAE 解码前。
- 几何分数独立投票为水印阳性。
- 攻击特定内容阈值或恢复后降阈值。
...
- 正式运行只允许一个登记 scheduler 注入索引。回调固定使用 Diffusers `callback_on_step_end`，索引从0开始；`z_i` 精确表示 scheduler step `i` 完成后、下一 step 开始前交给该回调的 latent。目标注入发生在20步采样的 callback 索引10，并以一次合成后的 latent 替换该回调返回值中的 latent；索引9只保存 `z_9`，不得写回。名义 replay 在 scheduler 全部完成后取得 terminal/pre-VAE latent，再执行一次独立的 HF-tail 终端载体写入并立即解码；它不是第二个 scheduler 注入时刻。
...
| HF tail 比例 | `0.20` |
| 正式归属检测权重 | `0.00 / 1.00`（LF / HF-tail） |
| 终端 HF-tail 路由、强度 | `semantic_unit_energy`、`8.0 × 0.0015 = 0.012` 相对 L2 |
| 局部敏感性探针相对步长 | `1e-3` |
| Q/K 最大抽样 token 数 | `64` |
...
LF 保留为 callback 索引10内容路由机制、质量预算和消融分支，但不再承担正式 registered/wrong-key 归属主证据。A100 因果诊断表明 LF-only 与旧 `0.70/0.30` dual 分数不能稳定区分 registered key；因此正式归属权重冻结为 `0.00`，LF 分数只允许作为诊断输出，不得反馈终端载体或最终裁决。
...
HF-tail 是正式密钥归属分支。索引10的原始 HF-tail 更新仍参与内容-几何双链；名义 replay 还必须在 terminal/pre-VAE latent 上以 `semantic_unit_energy` 路由写入固定 `8.0 × 0.0015` 相对 L2 的同密钥 HF-tail 载体。检测只对最终图像 VAE 重编码 latent 的 `s_HF-tail` 校准一个内容阈值，LF 不参与正式归属加权。

正式消融必须分别报告 LF-only、HF-tail-only 和旧 dual 内容链，用于解释传输损失，三者不得替代正式 terminal HF-tail 归属结果。LF-only 的诊断权重固定为 `1.0/0.0`，HF-tail-only 固定为 `0.0/1.0`；消融不得把结果反馈到正式终端写入的方向、密钥或阈值。
...
内容权重作为方法参数预先冻结为 `0.00/1.00`。calibration split 只能校准完整决策器的阈值和救回窗口，不得重新拟合内容权重；任何攻击、样本、profile 或结果均不得触发权重重选。旧 `0.70/0.30` 分数只作为 diagnostic-only 对照保存。
...
| terminal HF-tail 强度倍率 | `4.00, 8.00` | `8.00` |
| geometry 强度倍率 | `0.75, 1.00, 1.25` | `1.00` |

terminal HF-tail 倍率只作用于 scheduler 完成后的 `0.0015` HF-tail 相对强度；索引10的 LF/HF/geometry 名义强度保持 `0.0025/0.0015/0.0010` 不变。geometry 倍率只作用于 `0.0010`。Q/K 关系公式、捕获域、搜索网格、正式内容权重和 fixed-FPR 决策规则在敏感性实验中不得改变。
...
2. callback 索引10保留 LF/HF 分层路由，terminal/pre-VAE 的固定能量 HF-tail 载体承担正式密钥归属，LF/dual 只作为机制诊断和消融分数。
...
10. 三个分支在 callback 索引10只形成一次 scheduler latent 写回；名义 replay 另在 scheduler 完成后形成一次受独立固定能量约束的 terminal/pre-VAE HF-tail 写回。
