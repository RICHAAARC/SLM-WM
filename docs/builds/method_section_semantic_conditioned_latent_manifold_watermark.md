...
当 $u\notin\Omega_b$ 时，正式候选矩阵中的对应预算被置为 0；只有资格集合内部保留连续预算。该硬门控确保风险场会真实改变后续子空间，而不是仅作为日志字段。完整方法仅对当前活动且启用语义风险路由的分支执行空资格集合 fail-closed 门禁；移除风险路由的正式消融不使用风险阈值决定样本是否继续运行。
...
$$
F(z_t)=\left[F_{\mathrm{sem}}(z_t);F_{\mathrm{vis}}(z_t)\right],
...
$F_{\mathrm{sem}}$ 对应 VAE 可微解码与冻结 CLIP 图像编码器输出的512维归一化完整 embedding；$F_{\mathrm{vis}}$ 对应204维通道统计、梯度和8x8 RGB 空间向量。正式输出宽度为716，不执行降维投影。
...
正式实现直接调用模型的 `to_q` 与 `to_k` 投影，不使用合成 attention map。每个 token 的选择分数由跨冻结层 Q/K 关系行稳定度与接收 attention 显著度相乘得到, 固定选择前50%且不少于4个 token。一次注入的梯度、回溯和最终写回复验冻结同一选择集合。集合内关系权重为1, 其余规则网格关系保留0.25权重以支撑盲检二维注册。密钥确定 token 对与目标关系符号，形成注意力目标损失 $\mathcal L_A$。通过 autograd 得到
...
检测端只从待检图像、密钥和公开模型提取真实 Q/K 关系图。对冻结的有界相似仿射与方形二面体候选 $T$，构造把规范 token 坐标采样到观测坐标的双线性矩阵 $W_T$，并同时恢复 attention 的查询轴和键轴：

$$
\widehat A_T=W_T A_{\mathrm{obs}}W_T^\top.
$$

同时使用 $T^{-1}$ 构造从观测坐标采样规范密钥关系图的矩阵 $V_T$，得到该候选在观测参考系中应产生的关系图

$$
\widetilde S_T=V_TS_KV_T^\top.
$$

候选目标使用规范拉回相关分数 $s_{\mathrm{can}}$ 和观测前推相关分数 $s_{\mathrm{obs}}$：
...
$c$ 和 $u$ 分别表示有效坐标覆盖率与唯一采样率。观测前推项使用完整观测关系解释候选变换，防止只保留中心子图的尺度候选通过较小有效区域获得更高目标。检测器先执行冻结粗搜索，再围绕最优候选执行局部细化，并输出相对 identity 候选的观测关系增益、双向关系分数、覆盖惩罚、目标间隔、注册置信度、锚点内点比例和对齐残差。结构注册要求双向关系为正、目标间隔为正、两个方向的覆盖率均不低于0.45，并继续执行内点比例和残差门禁。得到 $\widehat T$ 后，检测器重采样待检图像并重新提取全部冻结层的真实 Q/K；恢复后同步分数必须通过 calibration split 冻结的阈值，才允许进入同阈值内容救回。几何链只负责参考系恢复和救回资格门禁，不独立产生 positive 判定。
...
7. 生成最终图像，执行 clean/watermarked 累计完整特征门禁，并记录算子成本、残差、投影能量和环境信息。
