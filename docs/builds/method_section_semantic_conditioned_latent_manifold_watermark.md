...
当前冻结阈值要求 $r_{\mathrm{written}}\leq10^{-4}$。该门禁直接约束真正进入 scheduler 的低精度 Tensor，不能由量化前方向的 Null Space 残差替代。随后，运行时还对每次实际写回执行完整特征有限更新复验；全部扩散步骤结束后，再直接比较最终 clean 与 watermarked 成图。两级有限变化门禁均要求 CLIP cosine 不低于0.995且手工结构统计特征相对漂移不高于0.02。
