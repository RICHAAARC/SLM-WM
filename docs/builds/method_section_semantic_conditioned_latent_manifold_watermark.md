...
$Q_0$ 只位于候选系数空间，不能直接当作 latent 基底。语义条件由 CLIP 图像嵌入的8个连续分块均值定义, 视觉条件由亮度、对比度、梯度和多尺度结构特征的8个连续分块均值定义。每个分支使用20个风险加权候选方向, 因而16维联合条件响应矩阵至少保留4维代数 Null Space。正式记录必须保存绝对响应残差 $\|R_bQ_0\|_F$、相对平均响应残差和正交误差 $\|B_b^\top B_b-I\|_F$。当前协议要求选中方向的平均响应不超过全部候选平均响应的0.0001，固定模板投影后的能量保留比例不低于0.01。门禁失败时直接停止运行，不能使用 one-hot、周期平铺或近零投影归一化作为替代。
...
候选目标由 $\widehat A_T$ 与密钥关系图的非对角归一化相关分数、有效坐标覆盖率和唯一采样率共同确定。检测器先执行冻结粗搜索，再围绕最优候选执行局部细化，并输出关系同步分数、目标间隔、注册置信度、锚点内点比例和对齐残差。得到 $\widehat T$ 后，检测器重采样待检图像并重新提取全部冻结层的真实 Q/K；恢复后同步分数必须通过 calibration split 冻结的阈值，才允许进入同阈值内容救回。几何链只负责参考系恢复和救回资格门禁，不独立产生 positive 判定。
