...
$\phi_{\mathrm{sem}}$ 表示语义显著性，$\phi_{\mathrm{tex}}$ 表示纹理复杂度，$\phi_{\mathrm{stab}}$ 表示跨采样步稳定性，$\phi_{\mathrm{sal}}$ 表示视觉显著性风险，$\phi_{\mathrm{attn\_stab}}$ 表示注意力关系稳定性。$\phi_{\mathrm{attn\_stab}}$ 必须由不少于两个冻结层的真实 Q/K 关系独立计算；它是注意力分支的必需输入，不能以 $\phi_{\mathrm{stab}}$、纹理图或常数替代。图像域语义掩码必须映射到 latent 分辨率并实际参与候选方向构造：
