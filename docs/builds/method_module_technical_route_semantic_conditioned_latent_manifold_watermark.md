...
映射回 latent 空间。正式记录同时保存响应残差、相对响应残差、正交误差和固定模板投影能量。当前门禁要求相对响应残差不超过 0.0001，内容模板能量保留比例不低于 0.01。
...
检测端从待检图像、密钥和公开模型提取真实 Q/K 关系图。对冻结的有界相似仿射与方形二面体候选构造双线性采样矩阵 $W_T$，以 $W_TA_{\mathrm{obs}}W_T^\top$ 同时恢复查询轴和键轴，并通过关系相关分数、覆盖率和唯一采样率完成粗搜索与局部细化。恢复图像参考系后必须重新提取全部冻结层的真实 Q/K，并由 calibration split 冻结注册分数、注册置信度和恢复后同步阈值。几何统计只能决定是否允许重对齐，不能独立给出 positive。
