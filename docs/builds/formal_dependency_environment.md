...
当前仓库登记了六个精确直接依赖输入和六个已提交完整哈希锁。每个锁必须逐项带 SHA-256、覆盖全部直接输入并产生有效逻辑摘要与正依赖计数; 任一锁缺失或发生工作树漂移时, 对应 profile 必须立即回到 `formal_ready=false`。锁文件通过仓库静态门禁只证明目标 wheel 闭包的受治理输入完整, 不证明实际环境已经安装或科学计算已经执行。

代码层的 fresh host 引导、精确 orchestrator child、科学 profile 目标 Python provision、候选审查包、Drive 回传接收、科学命令执行、解释器与依赖报告双向复验、执行证据绑定和归档门禁均使用 repository 路径。fresh-host 引导固定使用 `python -I`, 由宿主 Python 标准库下载并复验固定 PyPI Linux x86_64 `uv` wheel, 不把宿主 `venv`、`pip`、`ensurepip`、`PYTHONPATH`、用户 site-packages、激活环境或工具配置当作隐式输入; 精确 managed CPython 创建后才在其隔离环境中运行 `ensurepip`。当前没有可用的远程 Linux 服务器, 本地 Windows / CPU 环境不承担 Linux wheel 安装或 CUDA 执行。正式资格仍必须在后续 Colab 会话中通过父编排 preparation、科学子环境 preparation、完整 `pip check`、torch/CUDA identity、CUDA 可用性、对应硬件 smoke 与论文运行门禁。
