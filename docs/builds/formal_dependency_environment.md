...
本文档定义正式 Python 依赖环境的唯一协议。该协议回答以下问题:
...
| profile 运行时层 | `experiments/runtime/dependency_profiles.py` | 解析 registry、校验精确规格、计算稳定摘要、检查完整锁并核验目标解释器中的完整锁包集合。 |
| 目标解释器准备层 | `experiments/runtime/dependency_preparation.py` | 要求正式执行锁和目标完整锁, 执行 hash-locked 安装、兼容性检查与共享 inspection。 |
...
六个直接输入文件的精确条目数量和摘要必须从 dependency registry 与锁文件现场重建，不在本文手工冻结计数。父编排 profile 固定 `uv==0.11.28`、`huggingface_hub==1.20.1` 与 `numpy==2.0.2`; Hugging Face 客户端供父 runner 下载固定 revision 的模型和 OpenCLIP 快照, NumPy 供 CPU 结果闭合执行 Prompt 聚类 bootstrap、质量匹配比较和统计语义重建。五个科学执行 profile 均固定 `pip==24.3.1`、`setuptools==75.3.0` 和 `wheel==0.45.1`, 使目标环境中的安装器也进入完整锁证据。T2SMark 固定 `diffusers==0.32.0`; Gaussian Shading 固定 `scipy==1.10.1`、`huggingface_hub==0.25.2` 与 `transformers==4.33.3`, 该组合保留官方代码使用的 CLIP 接口并形成可解析的 Python 3.8 wheel 闭包。Shallow Diffuse profile 只登记固定官方入口及其导入图实际使用的包, 并要求全部传递依赖存在 Linux wheel, 使 `--only-binary=:all:` 闭包可逐项哈希。所有精确值只有在完整锁门禁与对应 CPU 或 GPU smoke 同时通过后才能表述为已资格化环境。
...
orchestrator 候选由精确 child 直接解析, 因而不会产生“先有 orchestrator 锁才能创建 orchestrator 候选”的循环依赖。科学 profile 只有在检出的提交必须包含 orchestrator 完整锁时才能继续: child 先以 `--require-hashes` 准备 orchestrator, 再由已验证的固定 uv 创建目标 CPython 子解释器并执行同一候选物化器。host launcher 最后重新读取 child manifest 与三个候选文件; 仅有返回码0但缺少文件、摘要不一致或 profile 身份漂移都不能形成成功报告。

Drive 回传后, 父编排候选由 `write_reviewed_dependency_hash_lock.py` 显式重复目标 profile, 并在候选生成提交对应的 clean Git HEAD 上重新验证正式代码锁、manifest、逐文件 SHA-256、pip report、全部直接输入、wheel URL、候选文本、逻辑摘要和依赖数量。父编排锁必须独立接收并提交, 使科学隔离解释器具备唯一可信的创建环境。随后五个科学 profile 可以基于同一新提交生成审查包, 并由 `write_reviewed_scientific_dependency_hash_locks.py` 按 registry 顺序逐项批准和原子接收。批量接收在写入前复验全部五个候选; 任一候选失败时不写任何科学锁, 任一写入失败时删除本次已写文件。两个接收器都只允许写 registry 登记且尚未物化的路径, 不覆盖已有锁、不执行 Git commit, 写入报告也不支持论文 claim。
...
4. 检查目标解释器和平台允许执行该 profile。
5. 使用完整锁执行 `--require-hashes` 和 binary-only 安装。
6. 五个科学执行 profile 使用目标 `sys.executable -m pip check`; 父编排 profile 将 `compatibility_check_required=false` 且记录 `not_applicable_to_orchestrator`。
...
`uv` 每个发行版内置一份冻结的可下载 Python distribution 列表。本协议固定 `uv==0.11.28` 并向 `uv python install` 与 `uv venv --managed-python` 传入完整 CPython patch, 不允许只指定 major/minor。provision 还要求实际 executable 路径属于父解释器安装的 `uv` distribution 文件清单, 且文件 SHA-256 与 `RECORD` 登记值一致; 仅在 PATH 中提供可报告相同版本的复制文件不能通过。报告保存 distribution `RECORD` 路径与摘要、executable 的 RECORD 路径与摘要、`uv --version` 结果、目标 Python 路径与摘要以及全部命令进程证据。正式 prepare 完成后再次计算目标 Python SHA-256, 防止安装期间解释器身份漂移。
...
1. Notebook 或普通服务器以 `python -I scripts/run_formal_workflow_host.py` 启动标准库宿主入口。该入口复验 clean detached commit, 从固定 SHA-256 的 `uv` wheel 创建精确 CPython 3.12.13, 再以已提交完整哈希锁准备父 `workflow_orchestrator`; 宿主 Python patch、宿主 `pip`、`venv` 和 `ensurepip` 均不参与正式环境身份。
2. 宿主使用精确父解释器执行 `scripts/formal_workflow_entry.py`。该入口与 `scripts/formal_workflow_environment.py` 均位于可独立服务器层, 不导入 `paper_workflow/`；GPU 路由、单 Prompt 方法资格化和 CPU 闭合由同一入口选择。
3. 主方法由 `scripts/semantic_watermark_scientific_workflow.py` 调用 `execute_isolated_scientific_command(...)`; method-faithful 与 T2SMark 由 `paper_experiments.runners.isolated_scientific_workflow` 调用同一隔离执行原语; 三个 official-reference runner 使用各自的隔离环境准备路径。每次 workflow 只准备一个科学子 profile。
4. 受验证子解释器执行完整方法、baseline 或 official-reference 科学 runner, 父解释器不导入 CUDA 方法实现.
5. 父解释器验证执行报告与 runner 产物, 将 workflow 环境、依赖报告快照和执行证据绑定到正式输出后再允许归档.

单 Prompt 方法资格化使用同一 fresh-host 链路, 但不进入正式批量 workflow。标准命令为:
...
目标“语义显著性自适应内容-几何双链潜空间水印”不使用 Jacobian、JVP/VJP 或 PSD-CG。目标依赖资格化只核验方法实际需要的 VAE 解码、CLIP patch-text 显著图、Sobel 纹理、相邻 latent 响应、公开探针敏感性、二维 LF/HF-tail 载体、真实 Q/K autograd、几何图像回正、单次 dtype 写回和图像 VAE 编码。迁移前兼容检查的实际退出状态只由项目构建状态文档登记。环境记录必须绑定 `torch_version`、`torch_cuda_version` 和 `execution_device_name`，且 `supports_paper_claim` 始终为 false。
...
脱离 Notebook 的 `scripts/run_gpu_server_workflow.py` 公开9个路由: 主方法、正式消融、三个 common-backbone method-faithful、T2SMark 和三个 official-reference 原环境复现。该入口直接调用 `scripts/formal_workflow_environment.py`, 因而运行层级、模型 identity、fixed-FPR 和持久化参数不依赖 Colab helper。所有路由复用上述 repository 模块与同一正式执行锁协议, 服务器入口不维护第二套科学实现。

## 10. 资格判定边界

六个依赖 profile 的直接输入、完整哈希锁、逻辑摘要和依赖计数必须由 registry、锁文件和环境 inspection 现场重建。任一锁缺失、输入摘要漂移、安装包不一致或目标硬件身份不满足时，对应 profile 必须 `formal_ready=false`。仓库中存在锁文件只证明受治理输入可以被复验，不证明实际环境已经安装或科学计算已经执行。

fresh-host 引导必须使用 `python -I`，由宿主 Python 标准库下载并复验固定 PyPI Linux x86_64 `uv` wheel，不把宿主 `venv`、`pip`、`ensurepip`、`PYTHONPATH`、用户 site-packages、激活环境或工具配置当作隐式输入。正式资格只能由目标 Linux/CUDA 环境中的父编排 preparation、科学子环境 preparation、完整 `pip check`、torch/CUDA identity、CUDA 可用性、对应硬件 smoke 与论文运行门禁共同形成。项目是否已经完成这些步骤只由 `project_construction_state.md` 和受治理资格化报告登记。
