...
将 attention geometry evidence 与可选 attention carrier 接入 same-threshold geometric rescue，完成几何恢复后内容重判。
...
1. stage09 geometry evidence manifest。
2. stage10 attention update manifest，如未稳定则使用 evidence-only fallback。
3. stage08 内容检测记录与 calibration 预备配置。
...
1. 在 `main/methods/detection/fusion.py` 与 `main/methods/geometry/recovery.py` 中完善 `positive_by_content`、`geometry_reliable`、`rescue_eligible`、`rescue_applied`、`evidence_decision`、`final_decision`。
2. 实现 rescue 窗口：`delta_low <= m_c_raw < 0`。
3. 实现 fail_reason gating：仅允许 `geometry_suspected` 与 `low_confidence`。
4. 对 `y_align` 或 aligned latent 重新运行内容检测，计算 `s_c_align` 与 `m_c_align`。
5. 实现 No-rescue、No-attention-anchor、FFT-sync-only、Image-registration-only、Geo-direct-positive audit。
6. 记录 rescue 后 clean negative 与 attacked negative 的整体误报统计，供 stage12 固定 FPR 审计使用。
...
1. 几何链不得直接判 positive。
2. 恢复后不得使用新的内容阈值。
3. `Geo-direct-positive audit` 不得进入正式方法。

## 七、产物与项目信息更新

本阶段应产出或更新以下内容：

1. `GoogleDrive/SLM-WM/runs/stage11_same_threshold_rescue/aligned_detection_records.jsonl`
2. `rescue_metrics_summary.csv`
3. `content_failed_subset_summary.csv`
4. `geometry_rescue_audit.json`
5. `manifest.json`
...
1. recovery 后仍使用同一个内容阈值。
2. rescue gain 能在 content failed subset 中解释。
3. No-rescue 等消融真实改变机制。
4. Geo-direct-positive audit 显示直接判正的 FPR 风险。
5. rescue 后 evidence-level FPR 必须能在 clean negative 与 attacked negative 上单独审计。

本阶段通过标准为：

几何链仅恢复参考系；rescue 通过同阈值内容重判完成；机制消融可运行。
