# 阶段7：semantic mask、risk field 与安全子空间正式实现

## 一、阶段定位

将 semantic mask 真实接入 latent trajectory feature operator，并实现 semantic-conditioned safe null-space。
...
3. `.codex/skills/placeholder_random_field_governance.skill.md`
...
1. stage06 event protocol manifest。
2. stage03 / stage04 的 latent trace、predicted x0、attention capture 或 final image artifact。
...
1. 在 `main/methods/semantic/` 中实现 `risk_field.py`、`routing.py`、`latent_mask.py`。
2. 在 `main/methods/subspace/` 中实现 `trajectory_features.py`、`jvp_estimator.py`、`safe_basis.py`、`route_projection.py`。
3. 在 runtime 中提供 mask 来源：predicted x0 saliency、final image saliency、segmentation model、Self-Attention saliency。
4. 确保 saliency model、segmentation model、SD attention capture 属于 runtime；core 只接收标准化 mask、latent mask 或 feature tensor。
5. 实现 `M_z = Pi_{x->z}(M_x)` 与 `varphi_t = P^T vec(Norm(M_z * z_t))`。
6. 生成 semantic route、basis digest、JVP / approximate JVP digest 与 mask projection report。

## 六、禁止事项与边界

本阶段不得执行以下操作：

1. 不得用 `M=1` 作为正式主方法；若保留，必须作为 `No-semantic-mask` 消融或 fallback。
2. 不得在 core 内加载 segmentation / saliency 模型权重。
3. 不得把未登记的 random / placeholder 字段用于正式 claim。

## 七、产物与项目信息更新

本阶段应产出或更新以下内容：

1. `GoogleDrive/SLM-WM/runs/stage07_semantic_subspace/semantic_route_records.jsonl`
2. `subspace_plan_records.jsonl`
3. `mask_projection_reports/`
4. `basis_digests.json`
5. `semantic_subspace_summary.json`
6. `manifest.json`
...
1. 不同图像产生不同 semantic route。
2. 关闭 semantic mask 会改变 basis。
3. `No-semantic-mask`、`Global-nullspace`、`Random-basis` 可运行。
4. semantic route、basis 和 JVP / approximate JVP 均有 digest。

本阶段通过标准为：

semantic mask 已真实影响 latent feature operator 与 basis；无语义条件的路径只能作为消融。
