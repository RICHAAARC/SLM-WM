# 阶段12：阈值校准与常规图像水印指标体系

## 一、阶段定位

建立 fixed-FPR calibration、clean negative / attacked negative 统计和常规图像水印评价指标体系。
...
1. 在 `experiments/protocol/calibration.py` 实现 fixed-FPR calibration。
2. 内容阈值由 calibration split 冻结：`tau_c=Q_{1-alpha}(s_c | D0^cal)`。
3. 实现 TPR、raw content clean FPR、rescue 后 clean negative FPR、rescue 后 attacked negative FPR、AUC、ROC / DET curve、BER / bit agreement、NCC / correlation、PSNR、SSIM、LPIPS、FID、KID、CLIP score、score retention、aligned score gain。
4. 冻结几何可靠性阈值、rescue window 和 fail reason gate，或确认它们已在前序协议中冻结。
5. 生成 score distribution table、threshold degeneracy report 与 rescue FPR audit。
6. 确保 test split 只报告结果，不调阈值、不调 rescue window、不调几何 gate。

## 六、禁止事项与边界

本阶段不得执行以下操作：

1. 不得用 test split 调阈值。
2. 不得混淆 clean negative 与 attacked negative。
3. 不得手工填写正式指标。
4. 不得只报告 raw content FPR 而省略 rescue 后整体 FPR。

## 七、产物与项目信息更新

本阶段应产出或更新以下内容：

1. `GoogleDrive/SLM-WM/runs/stage12_calibration_metrics/fixed_fpr_operating_points.csv`
2. `standard_watermark_metrics.csv`
3. `quality_metrics_summary.csv`
4. `roc_curve_points.csv`
5. `det_curve_points.csv`
6. `score_distribution_table.csv`
7. `threshold_degeneracy_report.json`
8. `manifest.json`
...
1. 所有正式阈值来自 calibration。
2. clean negative 和 attacked negative 分开统计。
3. score distribution 无明显阈值塌底或正负样本混杂未解释现象。
4. raw content decision 与 rescue 后 evidence-level decision 的 FPR 均可由 records 重建。
5. 若 rescue 后整体 FPR 超过目标 operating point，summary 必须降调 fixed-FPR 主张或标记为 unsupported。

本阶段通过标准为：

fixed-FPR 评价体系成立；阈值冻结；常规图像水印指标可重建。
