...
通过机制消融证明 SLM-WM 不是工程组件叠加，而是统一潜流形假设诱导出的完整方法。
...
1. stage12 calibration、stage13 attack matrix、stage14 baseline manifests。
2. 所有机制开关的配置定义。
...
1. 不得使用假开关。
2. 不得将 Geo-direct-positive audit 作为正式方法。
3. 不得只报告 Full 最高而不解释退化链。

## 七、产物与项目信息更新

本阶段应产出或更新以下内容：

1. `GoogleDrive/SLM-WM/runs/stage15_internal_ablations/ablation_records.jsonl`
2. `mechanism_ablation_table.csv`
3. `method_pairwise_delta_table.csv`
4. `ablation_by_attack_family.csv`
5. `ablation_claim_summary.json`
6. `manifest.json`
...
1. 每个消融有明确机制解释。
2. Full 方法形成可解释退化链，而不是简单堆模块。
3. 消融开关通过配置和记录可追踪。

本阶段通过标准为：

内部消融支撑统一潜流形方法主张；工程组合质疑可由表格和机制指标回应。
