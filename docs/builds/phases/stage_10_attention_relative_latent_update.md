# 阶段10：Attention-relative latent update

## 一、阶段定位

在 stage09 稳定后实现 attention-relative latent update，使几何锚点从检测证据升级为潜空间几何 carrier。
...
1. stage09 attention graph geometry manifest。
2. semantic safe basis、attention graph、stable token set 和 route projection。
...
1. 在 `main/methods/carrier/attention.py` 实现 attention-relative carrier。
2. 定义几何关系损失 `L_A = sum ||r_ij(z_t + Delta z_t) - r_ij^*||_2^2`。
3. 实现 `Delta z_t^A = -alpha_t^A Proj_Nsem(grad_z_t L_A)` 或可审计近似。
4. 实现 `attention_geometry_as_evidence_only` fallback，当真实 update 不稳定时将 attention 仅作为几何证据或补充机制。
5. 生成 update stability、quality metrics 与 relation consistency 曲线。

## 六、禁止事项与边界

本阶段不得执行以下操作：

1. 若 update 不稳定，不得写入 Full 方法主结果。
2. 不得为了提高 attention consistency 牺牲不可接受的语义质量。
3. 不得将 fallback evidence-only 路径声称为 attention watermark 主载体。

## 七、产物与项目信息更新

本阶段应产出或更新以下内容：

1. `GoogleDrive/SLM-WM/runs/stage10_attention_latent_update/attention_carrier_records.jsonl`
2. `attention_update_stability.csv`
3. `attention_update_quality_metrics.csv`
4. `attention_update_summary.json`
5. `manifest.json`
...
1. update 不导致图像崩坏。
2. attention relation consistency 随 update 强度有可解释变化。
3. CLIP / LPIPS / PSNR 等质量指标无不可接受退化。
4. 若失败，自动降级并记录 unsupported reason。

本阶段通过标准为：

attention-relative latent update 可稳定运行，或被明确降级为补充机制且不会污染 Full 方法主张。
