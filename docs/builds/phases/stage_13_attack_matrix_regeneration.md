...
建立常规攻击与再扩散攻击矩阵，产出 attack family metrics、score retention 与 rescue-by-attack 证据。
...
1. stage12 calibration metrics manifest。
2. event protocol manifest 与生成图像 manifest。
...
1. 实现常规攻击：JPEG compression、Gaussian noise、Gaussian blur、resize、crop、rotation、crop-resize、composite geometric attacks。
2. 实现再扩散攻击：img2img regeneration、DDIM inversion + regeneration、SDEdit regeneration、diffusion purification。
3. 采用资源档位：probe 检查 runner，pilot 确定攻击强度，full-main 常规攻击全量，full-extra 再扩散代表子集。
...
1. 再扩散攻击未经稳定性验证不得进入 full-main 主表。
2. 不得使用未登记 attack digest 的 attacked image。
3. 不得把不稳定的再扩散结果写入 abstract 核心主张。

## 七、产物与项目信息更新

本阶段应产出或更新以下内容：

1. `GoogleDrive/SLM-WM/runs/stage13_attack_matrix/attacked_images/`
2. `attack_manifest.json`
3. `attack_detection_records.jsonl`
4. `attack_family_metrics.csv`
5. `attack_strength_curve.csv`
6. `score_retention_by_attack.csv`
7. `rescue_by_attack.csv`
8. `manifest.json`
...
1. 每个 attack family 至少有 TPR、FPR、quality、score retention。
2. 所有 attacked images 有 attack digest。
3. full-extra 规模与主张边界在 summary 中明确。

本阶段通过标准为：

常规攻击全量路径成立；再扩散攻击具有代表子集或被明确降级。
