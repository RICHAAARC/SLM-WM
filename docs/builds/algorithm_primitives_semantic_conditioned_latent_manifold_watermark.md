...
SLM-WM 的完整特征算子固定连接512维归一化 CLIP `image_embeds` 与204维显式手工结构描述符。204维分量由颜色统计、梯度统计和8x8空间池化坐标组成, 是真实进入完整 Jacobian 的结构约束坐标；它不是通用感知质量模型、学习式感知 embedding、独立语义模型或仅用于记录的诊断 proxy。最终图像质量由实验层的 FID/KID、配对质量指标和必要的人工评价独立验证。

方法名称中的“潜流形”固定指当前 latent 点处716维特征水平集的一阶局部切空间解释。正式原语构造经分支风险支持和数值残差门控的 Jacobian 低响应子空间, 不学习或构造全局流形、坐标图、测地线、曲率、回缩或全局拓扑。局部性是当前算法原语的适用范围, 不是尚未完成的全局流形实现。

后续审计不得仅因204维坐标采用手工定义而将其判为代理实现, 也不得仅因方法不学习全局流形而将局部切空间判为不完整。审计应核验的是204维坐标是否按冻结协议真实计算、完整716维算子是否进入精确 JVP/VJP、局部低响应门禁是否执行, 以及论文主张是否保持在上述边界内。
...
其中，$F_{\mathrm{sem}}$ 是冻结 CLIP 输出的512维归一化完整 embedding；$F_{\mathrm{struct}}$ 是由3维通道均值、3维总体标准差、3维水平绝对梯度均值、3维垂直绝对梯度均值和192维8x8 RGB 平均池化组成的204维手工结构统计向量。VAE 输入严格执行 `latent / scaling_factor + shift_factor`, 解码结果执行 `decoded / 2 + 0.5` 并截断到 $[0,1]$。CLIP 输入固定为224x224 bicubic、`align_corners=false`、`antialias=true`, 随后使用冻结的逐通道 mean/std；512维来源必须是 `image_embeds`, 并沿最后一维执行非零能量 L2 归一化。204维严格按上述五个分块连接, 其中池化坐标按单样本 CHW 顺序展平；最终按512维在前、204维在后的顺序形成716维向量。正式 Jacobian 不执行坐标选择、随机投影或特征草图；任一输入、分量或输出非有限, 投影 embedding 能量为零, batch 不等于1或宽度漂移时均停止运行。该向量只声明对这些显式结构统计坐标的保持，不等同于一般感知质量模型；FID/KID 与配对质量指标在实验层独立评价最终图像。
...
完整解析配置以 `formal_method_config_schema=slm_wm_formal_method_runtime_config` 和 dataclass 的完整 `asdict` 值形成 payload, 通过 UTF-8、`ensure_ascii=false`、键名升序、无空白分隔符 `(',', ':')` 和 SHA-256 得到 `formal_method_config_digest`。该摘要不含 YAML 排版与绝对路径。运行时实际类名、VAE scale/shift、dtype、检测索引、冻结 attention 层和配置摘要必须全部一致；任一漂移都会改变 Jacobian、Null Space 或检测算子并使科学单元失败。
...
正式实现逐方向记录 PSD-CG 迭代数与残差、$\|J u_i^b\|_2/\|J B_b d_i^b\|_2$ 和平方二范数投影能量保留比例。设被接受的路由候选矩阵为 $V_b$、投影方向矩阵为 $U_b=N_bR_b$。float32 reduced QR 使用 `null_space_numerical_epsilon=1e-12` 规范列符号；任一对角项不大于该 epsilon 或条件数超过 `maximum_qr_condition_number=1000000.0` 时失败。独立参考严格采用 `qr_reference_solve_protocol=right_upper_triangular_solve_without_explicit_inverse`, 通过 $\widetilde V_bR_b=V_b$ 求解；第 $j$ 列残差使用 $J\widetilde V_b[:,j]$, 不能使用跨列共享 RMS。PSD-CG 不添加阻尼，最大64次迭代；只有右端项精确为零时允许零步返回, 其余情况必须用最终解重新执行线性算子并唯一要求真实相对残差不超过 $10^{-6}$。绝对残差和递推 residual 都不能替代该收敛定义。QR 后每列完整 Jacobian 相对残差不得超过 $10^{-4}$，能量保留比例不得低于0.01，且正交误差不得超过 `maximum_orthogonality_error=0.00001`。任一门禁失败时运行阻断。每个分支分别保存 $D_b$、$B_b$、$J(B_bD_b)$、$U_b$、$JU_b$、$N_b$、$JN_b$ 和 $J\widetilde V_b$ 的角色限定 Tensor 内容 SHA-256；每类响应只能使用其对应角色字段并绑定该数学对象。`solver_digest` 由候选与响应形状、秩、接受列索引、全部逐列数值和上述8类 Tensor 内容摘要通过共享纯记录函数重建；结果门禁必须重算, 不接受 producer 自报布尔值。
...
检测端只对待检图像执行 VAE 编码得到 $\hat z$, 并计算去均值归一化相关统计量。观测量与模板必须分别具有有限、非零的中心化能量；实现先分别归一化两个方向再计算内积, 常量或非有限输入直接失败：
...
公开二维坐标 $p_i\in[-1,1]^2$ 由 `token_indices` 唯一恢复。角点 token 中心分别落在 -1 与 1, 坐标身份固定为 `normalized_xy_token_centers_corner_endpoints`；token 稳定图插值和图像仿射重采样统一使用 `align_corners=True`。距离因子为
...
$W_i=0$ 或任一有限的加权中心化能量不大于 $\epsilon_{\mathrm{num}}^2$ 时该 row 无效。每个 batch 先对有效 row 取均值；没有有效 row 时该 batch 分量分数为0。该规则不能吸收 NaN 或 Inf；活动 Q/K、关系、投影、pair weight 和分数中出现非有限值时立即失败。随后依次对 batch 和冻结层等权平均：
...
注册关系分数记为 $s_{\mathrm{reg}}=s_{\mathrm{can}}(\widehat T,l)$。令规则抽样网格包含 $n$ 个 token, 正式方法预注册 $A=12$ 个锚点, 并按 $h_j=\operatorname{round}(j(n-1)/(A-1))$、$j=0,\ldots,11$ 在抽样索引范围内确定性均匀选择；$n<12$ 时直接失败, 不允许自动缩减锚点数。残差使用 `normalized_xy_token_centers_corner_endpoints` 中的归一化 xy 欧氏距离, token 关系采样与图像重采样统一采用 `align_corners=true`。仅具有有效双线性覆盖的锚点组成分母集合 $\mathcal V$；匹配到唯一观测 token 且残差 $d_j\le0.20$ 的锚点记为内点：
...
`attention_anchor_count=12`、`attention_residual_threshold=0.20` 与 `attention_minimum_inlier_ratio=0.50` 是预注册结构常量, 不是 calibration 或 test 选择的统计阈值。完整阈值无关 measurement 配置只保存在顶层运行 manifest；alignment 和样本记录保存连续测量原子及 measurement 配置摘要, calibration/test 必须绑定同一摘要。只允许在注意力几何开启时启用图像配准；关闭注意力几何而保留图像配准不是合法方法配置。启用几何救回时, `raw_attention_geometry_score`、`attention_geometry_score`、`registration_confidence`、`attention_sync_score` 和 `aligned_content_score` 五个连续原子必须同时存在且全部有限, alignment 与 aligned 内容也必须双向同时存在。measurement 中出现阈值、窗口或判定, 或任一测量原子缺失、非有限、数值漂移、配置摘要不一致, 均使当前闭合失败, 不得退回 raw 内容路径隐藏异常记录。

使用 $\hat T$ 重采样待检图像时, 输入先解码为 RGB uint8 Tensor 并归一化到 $[0,1]$；仿射网格和采样固定采用 bilinear、`padding_mode=border` 与 `align_corners=true`。连续结果按 $\operatorname{floor}(255\cdot\operatorname{clip}(x,0,1))$ 转回 RGB uint8。检测端随后必须重新运行同一个仅图像 Q/K 提取器, 并使用 $a_{\mathrm{can},\hat T}$ 对全部冻结层计算恢复后同步分数 $s_{\mathrm{sync}}$。恢复后不得重新选择稳定 token, 且观测、注册和同步记录的 pair 权重身份必须一致。同步分数缺失或非有限时当前测量立即失败, 不得静默退化为仅使用原始内容分数。最终几何可靠性定义为：
...
基础 latent 不调用各适配器自己的 CPU/CUDA RNG。协议使用 `sha256_counter_normal_icdf_table20_float32`, 把模型 ID、40位 revision、$s_{i,q}$、Tensor shape 和协议名称共同写入 domain, 从 SHA-256 大端计数器的连续 MSB-first 比特流提取20位索引并查询冻结 Q20 中点逆 CDF float32 表。规范 float32 生成和目标 dtype 转换均在 CPU 完成, 随后才搬运到目标设备：
...
单次执行只选择一个登记重复, 用于支持中断恢复和 GPU 会话隔离。正式论文汇总必须覆盖全部9个重复并把 Prompt、生成 seed 与密钥重复纳入统计组织；任一单重复结果都不能替代跨种子、跨密钥证据。`probe_paper`、`pilot_paper` 与 `full_paper` 使用同一个9重复注册表和同构 fixed-FPR 统计实现, 目标 FPR 分别为0.1、0.01和0.001, 同时改变下表中的 Prompt 数量、划分数量和统计强度。
