...
正式实现以8个 CLIP 语义分块条件和8个视觉统计分块条件组成联合 Jacobian, 并在20个风险加权候选方向上求解至少4维代数 Null Space。实现还必须记录 $\|R_bQ_0\|_F$、相对平均响应残差和 $\|B_b^\top B_b-I\|_F$, 分别验证绝对响应、相对响应与基底正交性。选中方向的平均响应不得超过全部候选平均响应的0.0001。固定盲检模板投影后的能量保留比例还必须不低于0.01; 低于门禁时运行失败, 不能对近零投影重新归一化后伪装为可检测载体。不得在 SVD 后直接施加未验证的坐标 mask, 因为后置 mask 可能破坏 Null Space 性质。
