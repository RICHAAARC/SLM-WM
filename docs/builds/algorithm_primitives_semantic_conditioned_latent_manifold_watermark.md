...
token 组成 $\mathcal{V}_A$。一次注入中的梯度、内容基底复算、单调回溯和最终
combined latent 复验必须冻结同一个集合。$\mathcal{V}_A$ 内关系权重为1, 其余
规则网格关系保留0.25权重, 以维持盲检双线性注册所需的完整二维采样支撑。
...
同时使用逆变换构造观测坐标到规范坐标的采样矩阵 $V_T$, 并生成候选变换在观测参考系中应呈现的密钥关系图

$$
\widetilde S_T^{(l)}=V_TS_K^{(l)}V_T^\top.
$$
...
s_{\mathrm{obs}}(T,l)=
\operatorname{corr}_{\mathrm{offdiag}}
\left(A_{\mathrm{obs}}^{(l)},\widetilde S_T^{(l)}\right).
$$
...
检测器先在冻结候选集合中粗搜索, 再围绕最优候选进行一次确定性局部细化。相对 identity 候选的可审计对齐增益为
...
注册关系分数记为 $s_{\mathrm{reg}}=s_{\mathrm{can}}(\widehat T,l)$, 锚点内点比例记为 $r_{\mathrm{inlier}}$, 内点平均残差记为 $e_{\mathrm{affine}}$。注册置信度为
...
使用 $\hat T$ 重采样待检图像后, 检测端必须重新运行同一个仅图像 Q/K 提取器, 并从全部冻结层计算恢复后同步分数 $s_{\mathrm{sync}}$。最终几何可靠性定义为：
