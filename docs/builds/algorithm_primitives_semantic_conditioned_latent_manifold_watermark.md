...
其中，$\phi_{\mathrm{sem}}$ 固定为 CLIP patch-to-CLS 余弦一致性，$\phi_{\mathrm{tex}}$ 固定为解码灰度绝对梯度和，$\phi_{\mathrm{lcr}}$ 固定为灰度相对反射填充5x5局部均值的绝对偏离；三者都双线性映射到 latent 网格并逐样本归一化。$\phi_{\mathrm{adj}}$ 使用当前与紧邻上一 scheduler 步的真实解码 RGB 差异：
...
正式实现逐方向记录 PSD-CG 迭代数与残差、$\|J u_i^b\|_2/\|J B_b d_i^b\|_2$、投影能量保留比例，并在 QR 后重新计算每列 $\|J N_b[:,j]\|_2$。PSD-CG 不添加阻尼，最大64次迭代，相对收敛阈值为 $10^{-6}$；QR 后每列完整 Jacobian 相对残差不得超过 $10^{-4}$，能量保留比例不得低于0.01，且基底正交误差不得超过 $10^{-5}$。任一门禁失败时运行阻断。每个分支还保存实际候选矩阵 $D_b$、风险预算对角项、响应矩阵 $JD_b$ 和 QR 后基底 $N_b$ 的精确 Tensor 内容 SHA-256。只有数值门禁与四个内容身份全部成立的记录才称为 Null Space。
...
这一复验用于捕获 float32 Null Space 方向转换为扩散 latent dtype 后产生的响应，验证对象是实际进入 scheduler 的增量。局部 Jacobian 零响应仍不能单独证明有限写回更新保持完整语义，因此实现还对每次真正写回的 latent 重新提取完整特征；全部扩散步骤结束后，再直接比较最终 clean 与 watermarked 成图。两级有限变化门禁均要求 CLIP cosine 不低于0.995且手工结构统计特征相对漂移不高于0.02。
...
分别执行逐行 pair 加权中心化和归一化相关, 再按冻结的非负归一化权重组合：

$$
c_c=\operatorname{RowCorr}_{w}(r^{(c)},\pi_cS_K),\qquad
s_A(z_t;K)=\sum_{c=1}^{4}\omega_c c_c,
\quad \omega_c\ge 0,
...
实现必须实际复算候选 latent 的 attention score, 通过单调回溯搜索保证接受的更新满足 `score_after > score_before`, 并记录 `score_before`、`score_after`、实际更新强度、回溯次数、原始梯度范数和投影后梯度范数。单次注入的 Q/K 原子角色固定为 `latent_before`、`content_base_latent`、`accepted_attention_candidate` 与 `actual_written_combined_latent`；四个角色必须逐层完整且联合摘要一致。若安全投影为零、回溯后仍不能提升真实 Q/K 目标或任一 Q/K 原子角色缺失, 运行必须失败。关系梯度 proxy 不得进入正式结果。
