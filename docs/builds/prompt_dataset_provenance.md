...
Prompt bank 中受外部来源登记约束的子集来自 Google Research 发布的 PartiPrompts：
...
| Prompt 集 | 本地固定子集 | PartiPrompts 登记子集 | 总数量 |
| --- | ---: | ---: | ---: |
| probe_paper | 60 | 10 | 70 |
| pilot_paper | 600 | 100 | 700 |
| full_paper | 6000 | 1000 | 7000 |

PartiPrompts 登记子集先使用小写和空白归一化执行去重, 再按来源 Category 比例分配
名额。每个 Category 内使用固定 SHA-256 排序选择。三个 Prompt 集的登记子集互不
复用, 并且不与本地固定子集中的任何规范化文本重复。由于 Prompt 文件
