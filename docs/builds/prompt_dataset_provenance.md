...
选择协议标识为 `nested_coco_parti_sha256_ranked_interleave`, 规则如下：
...
| --- | ---: | ---: | ---: | --- |
| `probe_paper` | 60 | 10 | 70 | `cdb7e573ea1cc90e39abb4b8fbb86a2a5458b4661449ab59e8c377258b0f7d59` |
...
完全不相交。`probe_paper`、`pilot_paper` 与 `full_paper` 分别使用 FPR=0.1、
FPR=0.01 与 FPR=0.001。三层运行共享同一方法、检测器、7项核心攻击、4个主表 baseline、消融、
统计代码与证据结构, Prompt 和 test 数量只控制样本规模, FPR 则定义各层独立的
错误接受率要求。10项补充攻击保持跨层配置身份同构，但只承担可选描述性职责，
不进入任一层级的 required claim gate。
