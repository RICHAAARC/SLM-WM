        ...
        rows.append(
            {
                ...
                else "sd35_latent_smoke_adapter_ready",
...
def write_smoke_outputs(tmp_path: Path) -> tuple[Path, Path]:
    """写出四个主表 baseline 的最小 smoke 命令结果和 observation。"""

    output_dir = tmp_path / "outputs" / "external_baseline_gpu_smoke" / "execution"
...
@pytest.mark.quick
def test_primary_baseline_evidence_distinguishes_smoke_from_formal_result(tmp_path: Path) -> None:
    """四个主表 baseline smoke 全通过时, 正式结果仍应因协议缺口保持未就绪。"""

    registry_path = write_source_registry(tmp_path)
    command_results_path, observations_path = write_smoke_outputs(tmp_path)
    ...
    assert summary["adapter_smoke_ready_count"] == 4
...
@pytest.mark.quick
def test_primary_baseline_evidence_writer_can_read_smoke_package(tmp_path: Path) -> None:
    """证据边界脚本应可直接读取 GPU smoke zip 包中的命令结果和 observation。"""
    ...
    with ZipFile(package_path, "w") as archive:
        ...
        archive.write(
            ...
            "outputs/external_baseline_gpu_smoke/execution/baseline_command_results.json",
        )
        archive.write(
            observations_path,
            "outputs/external_baseline_gpu_smoke/execution/baseline_observations.json",
        )
