...
此目录保存论文相关 Notebook 入口和执行环境包装。Notebook 文件统一位于 `paper_workflow/notebooks/`, 以避免入口文件与 helper 模块混放。Notebook 只能调度 repository modules, 不得成为唯一正式实现。
...
该变量只用于防止单个 baseline Notebook 误跑其他方法, 不表示只支持 Tree-Ring。其他运行配置不应继续散落在 Notebook cell 中。若需要修复配置、命令拼接、压缩包命名或 Google Drive 子目录, 优先修改 `paper_workflow/notebook_utils/`、`paper_workflow/colab_utils/` 或 `scripts/`, 不应把正式逻辑写入 Notebook。
...
- `attention_geometry_capture_run.ipynb`: 捕获真实 attention 载体或可审计 attention map, 生成 attention geometry 产物。正式调度逻辑位于 `experiments/runners/attention_geometry_capture.py`, Colab helper 仅保留旧导入路径兼容。
- `attention_latent_injection_run.ipynb`: 基于 attention geometry 执行 attention-relative latent update, 生成注入记录和配对质量记录。正式调度逻辑位于 `experiments/runners/attention_latent_injection.py`, Colab helper 仅保留旧导入路径兼容。
- `aligned_rescoring_run.ipynb`: 对注入结果进行 aligned rescoring, 生成检测重打分记录和配对感知质量指标。`probe_paper` 当前配置为60个 prompt, `pilot_paper` 当前配置为600个, `full_paper` 当前配置为6000个。正式调度逻辑位于 `experiments/runners/aligned_rescoring.py`, Colab helper 仅保留旧导入路径兼容。
- `threshold_calibration_run.ipynb`: 按当前运行层级的 fixed-FPR 协议校准阈值, 并记录 geometric rescue 边界。`probe_paper` 使用 FPR=0.1, `pilot_paper` 使用 FPR=0.01, `full_paper` 使用 FPR=0.001。正式调度逻辑位于 `experiments/runners/threshold_calibration.py`, Colab helper 仅保留旧导入路径兼容。
- `real_attack_evaluation_run.ipynb`: 调用 `experiments.runners.real_attack_evaluation` 生成或导入真实 attacked image, 执行正式攻击后检测, 记录 source / attacked image digest 和真实 GPU 攻击覆盖状态。该入口负责再扩散、全局编辑、局部编辑、视觉改写和自适应去水印类攻击。处理数量由当前论文运行层级的样本数派生。
- `conventional_geometric_attack_evaluation_run.ipynb`: 调用 `experiments.runners.conventional_geometric_attack_evaluation`, 使用 CPU / PIL 图像算子生成常规失真、几何变换与 photometric attacked image, 写出 source / attacked image digest、formal detection records 和总体进度条。正式检测口径需要对 attacked image 执行 SD3.5 detector / VAE latent rescore, 因此论文运行应使用 Colab GPU。该入口负责补齐 `standard_distortion`、`geometric_transform` 与 `photometric_distortion_attack` 的真实图像级攻击闭环。
- `dataset_level_quality_run.ipynb`: 从受治理图像集合导入 dataset-level 质量特征, 调用 `experiments.runners.dataset_level_quality` 计算或登记 FID / KID 等集合级指标。dataset-level 正式特征最小样本阈值由 prompt 数量派生, 当前为 `probe_paper=60`、`pilot_paper=100`、`full_paper=100`; 其结果只允许支撑当前运行层级样本规模内的主张, 不得跨运行层级提升。

### 外部 baseline 入口

method-faithful 入口把外部方法适配到 SD3.5 共同协议, 便于与主方法共享 prompt split、攻击矩阵和 fixed-FPR 校准边界。

method-faithful 的正式调度、共同攻击簇输出、observation 汇总与打包逻辑位于 `paper_experiments/runners/external_baseline_method_faithful.py`。`paper_workflow/colab_utils/external_baseline_method_faithful.py` 仅保留旧 Notebook 导入路径兼容, 不应承载唯一正式实现。
...
- `official_reference_t2smark_run.ipynb`: 运行 T2SMark 官方 SD3.5 路径并生成 governed import 候选记录。prompt 文件、样本数和目标 FPR 由当前论文运行层级派生。正式调度逻辑位于 `paper_experiments/runners/t2smark_full_main_reproduction.py`, Colab helper 仅保留旧导入路径兼容。
- `official_reference_tree_ring_run.ipynb`: 运行 Tree-Ring 官方原始环境参考复现, 样本数和 Drive 目录由当前论文运行层级派生。正式调度逻辑位于 `paper_experiments/runners/tree_ring_official_reference.py`, Colab helper 仅保留旧导入路径兼容。
- `official_reference_gaussian_shading_run.ipynb`: 运行 Gaussian Shading 官方原始环境参考复现, 样本数和 Drive 目录由当前论文运行层级派生。正式调度逻辑位于 `paper_experiments/runners/gaussian_shading_official_reference.py`, Colab helper 仅保留旧导入路径兼容。
- `official_reference_shallow_diffuse_run.ipynb`: 运行 Shallow Diffuse 官方原始环境参考复现, 样本数和 Drive 目录由当前论文运行层级派生。正式调度逻辑位于 `paper_experiments/runners/shallow_diffuse_official_reference.py`, Colab helper 仅保留旧导入路径兼容。
...
`probe_paper`、`pilot_paper` 与 `full_paper` 应共享同一批方法主流程、baseline 入口、攻击覆盖范围、bootstrap 设置、随机种子和结果闭合逻辑。运行语义只允许 prompt 数量与目标 FPR 不同; fixed-FPR clean negative 门禁由这两个量统一派生, 当前分别为 `probe_paper=10`、`pilot_paper=100`、`full_paper=1000`。Google Drive 输出根目录仅用于隔离落盘, 不构成实验协议差异。不得为 `probe_paper`、`pilot_paper` 和 `full_paper` 维护互相分叉的 Notebook 逻辑。`probe_paper` 用于小规模流程对齐验证; `pilot_paper` 是受样本规模约束的论文主张层级, 不是仅用于链路调试的临时层级。
...
切换运行层级时只修改这一行。依赖诊断由 `paper_workflow.colab_utils.dependency_check.build_notebook_dependency_report` 统一执行, archive 文件名和 Drive 镜像打包由 `paper_workflow.notebook_utils.notebook_entrypoint.package_workflow_outputs` 统一执行。Notebook 不再维护单独的短提交读取、UTC 后缀拼接或具体 `package_*_outputs` 调用, 以避免项目代码调试时同步修改多个 Notebook。
...
下表只描述 Notebook 入口之间的运行依赖。具体配置、路径、打包命名和依赖诊断仍由 `paper_workflow/notebook_utils/`、`paper_workflow/colab_utils/` 与 `scripts/` 统一实现, Notebook 不承载正式算法或统计逻辑; 外部 baseline、公平对比和受治理导入的正式实现应位于 `paper_experiments/`。
...
`paper_experiments/runners/paper_result_closure.py` 会生成并执行以下等价命令; `paper_workflow/colab_utils/paper_result_closure.py` 只负责 Colab 进度显示和 Notebook runtime 报告。若不使用结果闭合 Notebook, 也可以在同一个 Colab 仓库工作区或本地仓库中手动执行。若在本地执行, `--package-search-root` 应指向已经同步或挂载的 Google Drive `SLM/pilot_paper_results` 目录。
...
- `runtime_method_precheck_run.ipynb`: 运行时诊断入口调用 `experiments.runners.sd_runtime_cold_start` 与最小机制预检 runner, Notebook 仅负责 Colab 会话包装。
