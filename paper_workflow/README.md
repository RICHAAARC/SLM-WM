...
- `randomization_repeat_evidence_run.ipynb`: 单个 seed-key repeat 的7类 leaf 证据封装与 Drive 持久化入口。
...
各 GPU workflow 只在 `outputs/<artifact>/<paper_run_name>/` 写入正式产物并从该目录生成归档。单 repeat 证据选择器只消费7类活动随机化包, 重算运行锁、打包锁和 ZIP 字节摘要, 并要求全部 `code_version` 来源一致；3类跨 repeat 不变官方参考包留到9重复聚合层选择一次。Notebook 运行时间观测使用独立的 `outputs/notebook_runtime_observation/<paper_run_name>/` 路径, 因而不参与方法证据或结果包选择。
