...
| dataset-level 质量 | `dataset_level_quality_run.ipynb` | 建议 GPU; CPU 可运行但较慢 | `aligned_rescoring`, `real_attack_evaluation`, `conventional_geometric_attack_evaluation` | 必须等待攻击结果齐备 | `dataset_level_quality` 结果包, 包含 FID / KID 等集合级指标和输入图像登记。 |
