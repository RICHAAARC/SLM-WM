...
三个 `external_baseline_*_run.ipynb`、`official_reference_t2smark_run.ipynb` 和三个 `official_reference_*_run.ipynb` 均通过 `notebook_entrypoint.run_workflow` 进入同一个 repository 持久化会话.Notebook 不创建 checkpoint、不枚举结果文件, 也不判断某个中间文件是否具备论文资格.

1. `configure_paper_run_environment` 发布的 workflow Drive 目录同时保存正式 ZIP 与独立的 checkpoint 子目录.
...
host launcher 不假设 Colab 系统 Python patch。它使用 `configs/dependency_profiles/dependency_qualification_uv_linux_x86_64_lock.txt` 中唯一 Linux x86_64 wheel 的 SHA-256 安装固定 `uv==0.11.28`, 再创建 registry 登记的精确 CPython 3.12.13 orchestrator child。全部候选解析和审查包实现都在 repository 脚本中, Notebook 不包含安装命令、包清单、解释器创建或文件复制逻辑。

脚本始终在 `outputs/dependency_lock_review_bundles/<profile_id>/` 生成本地审查包。仅在显式提供 `--drive-output-dir` 时才复制到 Drive; 脱离 Notebook 的普通 Linux x86_64 服务器可以省略该参数。审查包包含候选锁、原始 `pip` resolver report、候选 provenance 和逐文件 SHA-256 manifest, 全部固定 `supports_paper_claim=false`。候选物化过程不写 `configs/`, `candidate_ready_for_review` 只表示文件可进入人工审查。
...
3. 提交 orchestrator 完整锁并重新检出该精确提交。此后才选择五个科学 profile。资格化 child 先按已提交哈希锁准备 orchestrator 环境, 再调用 `provision_isolated_dependency_python` 创建目标完整 CPython patch 的独立子环境, 最后由目标子解释器运行同一候选物化器。
...
5. 每个科学 profile 的审查包均使用同一接收命令, 并把两个 profile 参数同时改为当前目标。每次写入一个锁并提交 Git, 后续资格化必须检出包含已接收锁的新精确提交。
