...
def derive_minimum_clean_negative_count(prompt_count: int, target_fpr: float) -> int:
    """从样本规模和 fixed-FPR 标准派生 clean negative 门禁。

    该函数属于配置解析层。probe_paper、pilot_paper 与 full_paper 不再各自硬编码
    clean negative 门禁, 而是统一由 prompt 数量和目标 FPR 推导。这样三者除
    样本规模与 fixed-FPR 标准外, 不会引入额外协议差异。
    """
    ...
    fpr_resolution_count = int(math.ceil(1.0 / target_fpr))
    test_split_count = max(1, int(build_group_split_counts(prompt_count)["test"]))
    return max(1, min(fpr_resolution_count, test_split_count))
...
def build_paper_run_config(root: str | Path = ".") -> PaperRunConfig:
    ...
    derived_minimum_clean_negative_count = derive_minimum_clean_negative_count(prompt_count, target_fpr)
    derived_dataset_level_quality_minimum_count = derive_dataset_level_quality_minimum_count(prompt_count)
    return PaperRunConfig(
        ...
        minimum_clean_negative_count=int(
            os.environ.get(
                ...
                str(derived_minimum_clean_negative_count),
            )
        ),
        dataset_level_quality_minimum_count=int(
            os.environ.get(
                "SLM_WM_PAPER_RUN_DATASET_QUALITY_MINIMUM_COUNT",
                str(derived_dataset_level_quality_minimum_count),
            )
        ),
