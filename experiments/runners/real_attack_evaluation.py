"""真实再扩散与语义编辑攻击闭环的正式 runner。

该模块负责真实 SD3.5 Medium 图像攻击、攻击后检测、文件摘要登记和结果打包。
Colab Notebook 与服务器命令均调用这里的同一套实现, 不在入口层伪造 GPU 结果。
"""
...
PACKAGE_EXTRA_PATHS = (
    "experiments/runners/real_attack_evaluation.py",
...
def write_failure_outputs(
    ...
    manifest = build_artifact_manifest(
        ...
        rebuild_command="调用 experiments.runners.real_attack_evaluation",
