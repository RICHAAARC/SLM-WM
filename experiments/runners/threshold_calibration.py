...
from experiments.artifacts.geometric_rescue_outputs import write_geometric_rescue_outputs
from experiments.artifacts.threshold_calibration_outputs import write_threshold_calibration_outputs
...
PACKAGE_EXTRA_PATHS = (
    ...
    "experiments/artifacts/geometric_rescue_outputs.py",
    "experiments/artifacts/threshold_calibration_outputs.py",
)
...
def write_failure_outputs(root_path: Path, error: Exception) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "rebuild_command": "调用 experiments.runners.threshold_calibration",
    }
    manifest_path.write_text(stable_json_text(manifest), encoding="utf-8")
    return result
...
def run_default_threshold_calibration_from_drive_plan(
    ...
    minimum_clean_negative_count: int | str | None = None,
) -> dict[str, Any]:
    ...
    result = {
        ...
        "metadata": {
            ...
            "calibration_fpr_exceeds_target": threshold_report.get("calibration_fpr_exceeds_target", False),
            "test_clean_fpr_exceeds_target": threshold_report.get("test_clean_fpr_exceeds_target", False),
            "formal_detection_test_clean_fpr_exceeds_target": threshold_report.get(
                "formal_detection_test_clean_fpr_exceeds_target",
                False,
            ),
            ...
            "workflow_calibration_ready": threshold_report.get("workflow_calibration_ready", False),
            "paper_claim_empirical_fpr_ready": threshold_report.get("paper_claim_empirical_fpr_ready", False),
            "formal_detection_claim_ready": threshold_report.get("formal_detection_claim_ready", False),
