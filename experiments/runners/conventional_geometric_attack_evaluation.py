"""常规失真与几何变换真实图像级攻击闭环 runner。

该模块执行不依赖 diffusion 生成的图像攻击, 例如 JPEG、噪声、模糊、缩放、裁剪、旋转和光照颜色扰动。
Colab Notebook 与服务器命令均调用这里的同一套实现, 正式记录、注册表、metrics 和 manifest 不在入口层手写。
"""
...
from experiments.runtime.repository_environment import build_runtime_environment_report, resolve_code_version

DEFAULT_OUTPUT_DIR = "outputs/conventional_geometric_attack_evaluation"
DEFAULT_DRIVE_OUTPUT_DIR = ""
PACKAGE_EXTRA_PATHS = (
    "experiments/runners/conventional_geometric_attack_evaluation.py",
...
def write_failure_outputs(
    ...
    manifest = build_artifact_manifest(
        ...
        rebuild_command="调用 experiments.runners.conventional_geometric_attack_evaluation",
