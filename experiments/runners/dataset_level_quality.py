"""dataset-level quality 服务器与 Notebook 共用 runner。

该模块负责把真实攻击包与 aligned rescoring 包接入正式特征导入协议。
Notebook 只负责挂载远程存储与调用这里的 runner, 不在 cell 中直接拼接正式 records 或报告。
"""
...
PACKAGE_EXTRA_PATHS = (
    "experiments/runners/dataset_level_quality.py",
...
def write_failure_outputs(root_path: Path, error: Exception) -> dict[str, Any]:
    ...
    manifest = build_artifact_manifest(
        ...
        rebuild_command="调用 experiments.runners.dataset_level_quality",
        metadata=result,
    ).to_dict()
    manifest_path.write_text(stable_json_text(manifest), encoding="utf-8")
    return result
