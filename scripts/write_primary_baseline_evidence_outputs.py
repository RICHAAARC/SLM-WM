...
DEFAULT_COMMAND_RESULTS_PATH = Path("outputs/external_baseline_gpu_smoke/execution/baseline_command_results.json")
...
PACKAGE_COMMAND_RESULTS_ENTRY = "outputs/external_baseline_gpu_smoke/execution/baseline_command_results.json"
...
def write_primary_baseline_evidence_outputs(
    *,
    ...
    """写出主表 baseline smoke 证据与正式结果缺口记录。"""
    ...
    if resolved_smoke_package_path:
        source_registry = load_optional_json(resolved_source_registry_path) or load_json_from_package(
            resolved_smoke_package_path,
            PACKAGE_SOURCE_REGISTRY_ENTRY,
        ) or {}
        command_results = load_json_from_package(resolved_smoke_package_path, PACKAGE_COMMAND_RESULTS_ENTRY) or []
        observation_rows = load_json_from_package(resolved_smoke_package_path, PACKAGE_OBSERVATIONS_ENTRY) or []
    else:
        source_registry = load_optional_json(resolved_source_registry_path) or {}
        command_results = load_optional_json(resolved_command_results_path) or []
        observation_rows = load_optional_json(resolved_observations_path) or []
