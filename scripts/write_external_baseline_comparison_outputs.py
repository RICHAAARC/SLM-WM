...
def write_external_baseline_comparison_outputs(
    ...
    baseline_specs = default_baseline_specs()
    boundary = attack_manifest.get("evaluation_boundary", {})

    observations = build_baseline_observations(baseline_specs, attack_rows, boundary)
    ...
    runtime_report = build_runtime_report(attack_manifest, threshold_report, baseline_metric_rows, observations)
    ...
    output_paths = tuple(
        ...
        for path in (observations_path, metrics_path, comparison_path, runtime_report_path, manifest_path)
    )
    input_paths = (
...
def build_parser() -> argparse.ArgumentParser:
    """构造命令行参数解析器。"""
    parser = argparse.ArgumentParser(description="写出外部 baseline 公平对比协议产物。")
