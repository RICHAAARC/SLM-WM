...
仅图像注意力配准使用 `formal_method_config_schema=slm_wm_formal_method_runtime_config_v4` 中的唯一结构门禁: `attention_anchor_count=12`、`attention_residual_threshold=0.20`、`attention_minimum_inlier_ratio=0.50`。锚点按抽样 token 索引确定性均匀选择；token 数少于12时失败。残差采用上述归一化 xy 坐标中的欧氏距离, 内点率只以具有有效双线性覆盖的锚点为分母并要求唯一观测匹配。三项值为预注册方法常量, calibration/test 只能冻结或应用内容分数、几何关系分、注册置信度、恢复后同步分和 rescue window, 不得调整结构门禁。完整方法配置只由顶层运行 manifest 保存；alignment 和样本记录保存决策所需字段及检测器配置摘要, calibration/test 必须绑定同一摘要。
