...
逐攻击质量 estimand 位于 `configs/attack_conditioned_quality_estimand.json`。基础比较固定为 clean 对 watermarked, 攻击条件比较固定为同一攻击配置和同一 `attack_seed_random` 下的 attacked-clean 对 attacked-watermarked。CLIP 固定使用 `openai/clip-vit-base-patch32` 的登记 revision 和归一化 `image_embeds`, 分布质量固定使用 Prompt 条件 KID。

`configs/keyed_prg_cross_platform_known_answer.json` 保存公开 uniform/Gaussian 固定输入及 Tensor 内容摘要。该文件用于 Linux/Colab 对 Windows 冻结向量执行逐字节复验, 不包含论文统计结果, 也不直接支持论文主张。
