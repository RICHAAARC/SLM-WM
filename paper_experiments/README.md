...
- `runners/closure_package_selection.py`: 按10类包内身份、当前论文运行层级、fixed-FPR、baseline、ready 状态和 `outputs/` 白名单选择精确输入, 并写出 run-scoped 输入锁与独立 manifest。完整18步服务器闭合编排位于外层 `scripts/paper_result_closure.py`, 本层只提供输入选择、正式实验和证据分析能力。
...
- `runners/external_baseline_method_faithful.py`: 单个 Tree-Ring、Gaussian Shading 或 Shallow Diffuse 的 SD3.5 common-backbone 适配调度、逐 Prompt 原子完成单元复算、fixed-FPR 审计、transfer manifest 和独占结果打包逻辑。断点恢复发生在真实 adapter 内部; runner 保留完成单元并清除可重建派生文件, 然后再次验证 test Prompt 攻击 exact set 与跨会话科学来源。
...
- `analysis/formal_record_statistics.py`: 最终闭合不信任机制必要性 summary、必要性 CSV 或 FID/KID 指标表。该模块从逐 Prompt 正式消融重运行记录重新执行配对 bootstrap、Holm 校正和质量非劣门禁, 并从逐图像正式 Inception feature records 以 float64 独立重算 FID/KID, 随后逐字段比较全部派生行。未获支持的单机制结论仍可作为真实负结果进入协议闭合, 但任何原始记录、派生统计或指标字段漂移都会 fail-closed。
...
主方法、method-faithful、T2SMark 与三套 official-reference 的完整科学 runner 均由五个登记 CUDA profile 的隔离子解释器执行. 当前唯一外部阻断类别是目标完整哈希锁资格审查尚未闭合, 五个科学锁必须在匹配各自环境身份的 Colab 或 Linux CUDA runtime 完成审查后才能产生正式输入包.
