...
1. 从不可变单 repeat 证据组件重建精确9重复聚合来源，拒绝缺失、重复、额外或身份漂移成员。
...
5. `probe_paper`、`pilot_paper` 和 `full_paper` 固定使用相同方法、攻击、baseline、统计程序、产物 schema 和精确9重复，只允许登记规模、目标 FPR、统计强度及其派生记录数量变化。
6. official-reference 证据只承担登记的方法忠实度职责，不替代 common-backbone 主表比较。
