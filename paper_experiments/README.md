...
- `runners/randomization_paired_superiority.py`: 只接收同一个不可变聚合来源对象, 使用上述45个原始来源和45个逐 repeat 阈值分别执行36次主方法与 baseline 配对, 并强制主方法攻击后记录通过仅图像盲检原子复算、两方法均绑定真实攻击图像。该入口随后把9重复作为同一 Prompt 的重复测量, 以34 / 340 / 3400个 test Prompt 为独立统计单位执行100000次 bootstrap、Hoeffding 检验和4个主表 baseline 的 Holm 校正；可通过 `python -m paper_experiments.runners.randomization_paired_superiority` 脱离 Notebook 事务写出45阈值、样本级 outcome、统计表、summary、报告与 manifest。
...
- `analysis/randomization_paired_superiority.py`: 对9个注册 repeat 的配对 outcome 执行精确笛卡尔积校验。总体效应先在每个 repeat 内跨完整攻击集合平均, 再在同一 Prompt 内跨9重复平均, 最后只以 Prompt 数量执行 bootstrap 和 Hoeffding 推断；4个 baseline 构成唯一 Holm 家族, `paired_observation_count` 只作为原子观测数量, 不作为推断样本量。
