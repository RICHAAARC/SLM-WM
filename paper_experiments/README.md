...
- `runners/t2smark_formal_reproduction.py`: T2SMark 官方 SD3.5 路径复现、prompt split 导出、固定 FPR 候选记录生成、受治理导入 校验和结果打包逻辑。
- `runners/tree_ring_official_reference.py`: Tree-Ring 官方参考复现、官方依赖环境准备、模型仓库布局核验、受治理导入 记录和结果打包逻辑。
- `runners/gaussian_shading_official_reference.py`: Gaussian Shading 官方参考复现、官方依赖环境准备、严格官方依赖优先策略、受治理导入 记录和结果打包逻辑。
- `runners/shallow_diffuse_official_reference.py`: Shallow Diffuse 官方参考复现、官方依赖环境准备、源码运行边界核验、受治理导入 记录和结果打包逻辑。
