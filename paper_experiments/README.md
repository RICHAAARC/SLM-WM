...
- `runners/closure_package_selection.py`: 按包内身份、当前论文运行层级、fixed-FPR、baseline、ready 状态和 `outputs/` 白名单选择精确输入。单 repeat 路径只选择8类活动随机化 leaf package; 3类跨 repeat 不变官方参考包由聚合路径单独选择。论文闭合编排位于外层 `scripts/paper_result_closure.py`, 本层提供输入选择、正式实验和证据分析能力。
...
- `runners/randomization_method_repeat_thresholds.py`: 只接收同一个不可变聚合来源对象, 将9个 runtime records 逐条绑定到包内 Prompt 契约, 重新核对精确 runtime schema、完整正式方法配置、split、模型、生成 seed 和运行前预注册的3-key 计划, 在 CPU 上按版本化 PRG 重新生成3组基础 latent；同时核验4个 baseline 顶层运行 manifest 保存的同一完整9重复计划与仅图像检测访问模式, 再把45组 observation 与 producer 阈值声明交给纯分析层逐一重算。该桥接入口不接受路径、Prompt 或成员覆盖, 并以同一个内存重建对象向后续统计提供45个已验证原始来源；重建报告固定 `supports_paper_claim=false`。
- `runners/randomization_paired_superiority.py`: 只接收同一个不可变聚合来源对象, 使用上述45个原始来源和45个逐 repeat 阈值分别执行36次主方法与 baseline 配对, 并强制主方法攻击后记录通过仅图像盲检原子复算、两方法均绑定真实攻击图像。该入口从相同来源另行构造 `9 x 4 x test Prompt` 条未攻击 SSIM 质量记录, 不把质量字段复制到攻击 outcome。全样本和质量匹配统计都把9重复作为同一 Prompt 的重复测量, 以34 / 340 / 3400个 test Prompt 为独立统计单位执行100000次 bootstrap、Hoeffding 检验和各自独立的4项 Holm 校正；可通过 `python -m paper_experiments.runners.randomization_paired_superiority` 脱离 Notebook 事务写出45阈值、样本级 outcome、质量记录、4行合并主表、summary、报告与 manifest。
- `runners/randomization_dataset_quality.py`: 只接收同一个不可变聚合来源对象, 从9份质量 leaf 读取完整 Prompt 集合的 clean-watermarked 图像记录和原始 Inception 特征。每份记录先核验图像对自摘要、Prompt 精确集合、source/comparison 角色、正式2048维提取器和真实 CUDA 科学完成单元来源, 随后把 `9 x 70 / 700 / 7000` 个特征对联合输入冻结 float64 FID 与 KID 算子。该入口不读取或平均单 repeat 派生指标表, 可通过 `python -m paper_experiments.runners.randomization_dataset_quality` 脱离 Notebook 事务写出 FID/KID 三行、最小成员关系、summary、报告与 manifest。
- `runners/randomization_ablation_necessity.py`: 只接受同一个不可变聚合来源对象, 从9份消融 leaf 分别读取真实重运行记录、检测原子、冻结协议和顶层运行 manifest。每个 repeat 先独立重算 calibration 阈值与逐 Prompt 检测聚合, 随后在同一 Prompt 内平均9个注册 repeat 的完整方法减变体效应, 最后以34 / 340 / 3400个 test Prompt 为聚类单位执行 bootstrap、Hoeffding 检验、Holm 校正和 paired SSIM 非劣门禁。该入口不读取单重复必要性 CSV 或 summary, 可通过 `python -m paper_experiments.runners.randomization_ablation_necessity` 脱离 Notebook 写出最小跨重复消融结果与 manifest。
...
- `analysis/paired_superiority.py`: 在单个注册 repeat 内对 SLM-WM 与4个主表 baseline 的完整 test Prompt x attack 观测执行精确配对, 核验攻击 seed、生成 seed、密钥、基础 latent、冻结阈值和攻击后图像检测事实；其中的单 repeat 统计与质量匹配算子不承担跨重复论文结论。
- `analysis/randomization_paired_superiority.py`: 对9个注册 repeat 的配对 outcome 和检测标签无关质量记录分别执行精确笛卡尔积校验。总体效应先在每个 repeat 内跨完整攻击集合平均, 再在同一 Prompt 内跨9重复平均, 最后只以 Prompt 数量执行 bootstrap 和 Hoeffding 推断。质量匹配要求同一 Prompt 的9个 repeat 全部满足 SSIM caliper, 随后纳入该 Prompt 的完整 `9 x attack` 检测块；全样本与质量匹配各自保持固定4项 Holm 家族, 原子观测数量不作为推断样本量。
- `analysis/randomization_dataset_quality.py`: 要求质量成员精确形成9个登记 repeat 与完整 Prompt 集合的笛卡尔积, 且每个成员只连接一个唯一图像对和两个正式特征角色。统计层按规范成员顺序从原始特征联合重算 FID、KID mean 与 KID std；输入记录顺序、单 repeat 指标表或 ready 字段均不能改变数值。三行完成测量表示质量统计可重建, 不自动表示论文效果结论获支持。
...
- 精确9个 repeat 组件与3个跨 repeat 不变 official-reference 包进入聚合来源前, 生产 validator 会独立核验 official-reference 精确集合、共同代码提交和数值忠实度资格。最终统计 Writer 只消费该不可变聚合来源, 不重复复制 official-reference 结果。
...
七个活动随机化 family 与三个 official-reference family 均把正式运行写入 `outputs/<artifact>/<paper_run_name>/...`。主方法、质量、消融、单模型内部风险参数敏感性、三个 common-backbone baseline 与 T2SMark 共8类随机化证据包, 必须由 package input 和 archive 或运行 manifest 两个独立来源共同绑定活动 `randomization_repeat_id`、seed/key 索引和随机化协议摘要。Tree-Ring、Gaussian Shading 与 Shallow Diffuse 的3个 official-reference 原环境包只承担跨 repeat 不变的数值忠实度证据, 不得伪装成随机化主表结果。归档名称包含方法身份和唯一时间后缀, 包内身份还必须绑定同一 `paper_run_name`、`target_fpr`、`baseline_id` 和 `code_version`。归档白名单只允许当前 family 的 `outputs/` 成员, 不把 runner 源码、README、source registry 或 Notebook runtime 报告复制进正式输入包。

单 repeat 输入选择与自包含证据包固定 `repeat_component_ready=true`、`randomization_aggregate_ready=false` 和 `supports_paper_claim=false`。该组件只证明当前 seed-key 单元的8类随机化包没有跨 repeat 混选; 3类不变忠实度包不复制进 component, 由聚合层独立选择一次。最终论文闭合必须精确覆盖权威9 ID 并从原始观测与特征重算统计。
...
- 单个 Prompt 在每个 repeat 内的效应值是跨完整攻击集合的平均二元检测差值, 随后对同一 Prompt 的9个注册 repeat 等权平均。正式统计使用100000次 Prompt-clustered bootstrap 的95% percentile CI, 每次重采样都整体移动一个 Prompt 的 `9 x attack` 观测块；对取值位于 [-1,1] 的 Prompt 聚类差执行单侧 bounded Hoeffding 均值检验, 再对4个主表 claim p 值执行 Holm 校正。推断样本量固定为34 / 340 / 3400个 test Prompt, 不得扩张为306 / 3060 / 30600个 repeat-Prompt 单元。整数动态规划 exact sign-flip 只披露 sharp-null 诊断, 不参与均值优势 claim 门禁。
- 质量匹配使用未攻击 clean-watermarked pair 的实测 `embedding_pair_ssim`, 并逐 repeat 绑定相同 Prompt、生成 seed、水印密钥与基础 latent 身份。对固定 baseline 和 Prompt, 只有9个注册 repeat 的绝对 SSIM 差都不超过0.02时, 才纳入该 Prompt 的完整 `9 x attack` 检测块；不得平均9个质量差来掩盖单个 repeat 越界, 也不得只保留部分匹配 repeat。每个 baseline 至少覆盖80%的完整 test Prompt, 即 probe / pilot / full 分别至少28 / 272 / 2720个。不同 baseline 可以形成不同子集。
...
每个 repeat 由 `randomization_repeat_evidence.py` 精确选择8类活动随机化 leaf ZIP, 保持 leaf 原始 ZIP 字节并写入独立的自包含 evidence package。3类跨 repeat 不变的 official-reference 忠实度包不复制进每个 repeat, 只在聚合层选择一次。单 repeat package 的 manifest 绑定 seed/key 身份、随机化协议、代码版本、执行锁摘要与8个 leaf SHA-256, 并固定 `repeat_component_ready=true`、`randomization_aggregate_ready=false`、`supports_paper_claim=false`。

聚合 package 的 manifest 以 `{aggregate_package_path}` 明确标记调用方必须提供的当前来源 ZIP, 并登记 `repository_root` 工作目录与 `self_contained_aggregate_zip` 输入模式。层内重建入口先完整验证全部登记成员, 再把9个 repeat component 与3个 invariant ZIP 提取到临时目录并重新调用生产 validator；重建命令不假定内部成员路径已经存在于宿主文件系统。
...
- `probe_paper`、`pilot_paper` 和 `full_paper` 都运行同一完整论文方法、攻击、baseline、消融、统计和证据 schema。三级目标 FPR 分别冻结为0.1、0.01和0.001, Prompt / test 数量分别为70 / 34、700 / 340和7000 / 3400。
