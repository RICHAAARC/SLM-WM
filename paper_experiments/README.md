...
单 repeat 输入选择固定 `repeat_component_input_ready=true`、`randomization_aggregate_ready=false` 和 `supports_paper_claim=false`。该选择只能证明当前 seed-key 单元的7类随机化包与3类不变忠实度包没有跨 repeat 混选, 不能替代9重复聚合。最终论文闭合必须由后续 aggregate 路径精确覆盖权威9 ID 并从原始观测与特征重算统计。
...
单 repeat 的10包选择是 repeat component 输入治理步骤, 不是最终 CPU 论文闭合。最终 aggregate DAG 必须在精确9个 component 全部存在后执行 official-reference 忠实度审计、攻击矩阵、45个 method-repeat fixed-FPR 阈值审计、Prompt 聚类配对优势、主表证据、正式结果 records、共同协议、结果分析、论文表图实际数据审计、投稿就绪审计、证据闭合入口审计、结果闭合语义门禁和完整结果包打包。任一 repeat 缺失、重复、身份漂移或原始记录重算失败都必须阻断后续闭合。
