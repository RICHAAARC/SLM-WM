...
def _baseline_protocol_compatible(spec: BaselineSpec, attack_row: Mapping[str, Any]) -> bool:
    ...
    try:
        ...
        return False
    return bool(
        spec.baseline_adapter_ready
        and _str_field(attack_row, "attack_id") == config.attack_id
        and _str_field(attack_row, "attack_config_digest")
...
def normalize_baseline_result_record(row: Mapping[str, Any]) -> BaselineResultRecord:
    ...
    payload = {
        ...
        "attack_config_digest": _str_field(row, "attack_config_digest"),
        ...
        "evidence_paths": tuple(
            str(value) for value in row.get("evidence_paths", ()) if str(value).strip()
        ),
    }
    config = resolve_formal_attack_config(
    ...
    if (
        payload["attack_id"] != config.attack_id
        or payload["attack_config_digest"] != attack_config_digest(config)
    ):
        raise ValueError("baseline result record 攻击身份与正式配置不一致")
...
) -> dict[tuple[str, str, str, str, str, str, str], BaselineResultRecord]:
