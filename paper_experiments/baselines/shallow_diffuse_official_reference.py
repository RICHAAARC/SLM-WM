...
if len(_OPENCLIP_SOURCE.required_files) != 1:
    raise RuntimeError("Shallow Diffuse OpenCLIP 登记源必须声明唯一 checkpoint")
...
class ShallowDiffuseOfficialReferenceIssue:
    """记录 Shallow Diffuse 官方参考导入中的单个 schema 问题。"""
...
def _scientific_config_fields(
    ...
    """读取并复验与 Shallow Diffuse 科学单元同源的规范配置."""
...
def build_shallow_diffuse_official_reference_schema() -> dict[str, Any]:
    """构造 Shallow Diffuse 官方参考导入 schema 描述。"""

    return {
        "reference_protocol_name": SHALLOW_DIFFUSE_OFFICIAL_REFERENCE_PROTOCOL_NAME,
        "baseline_id": "shallow_diffuse",
        "supplemental_table_role": SHALLOW_DIFFUSE_SUPPLEMENTAL_TABLE_ROLE,
...
def build_shallow_diffuse_official_reference_record(
    *,
    official_entrypoint: str,
    official_repository_commit: str,
    official_environment_profile: str,
    ...
    missing_metric_fields = tuple(
        ...
        if field_name not in metric_values or metric_values[field_name] is None
    )
    if missing_metric_fields:
        raise ValueError(f"Shallow Diffuse 官方记录缺少科学指标: {missing_metric_fields}")
    ...
    payload = {
        ...
        "openclip_checkpoint_size_bytes": _int_field(
            source_provenance,
            "openclip_checkpoint_size_bytes",
        ),
        "openclip_snapshot_content_digest": _str_field(
            source_provenance,
            "openclip_snapshot_content_digest",
        ),
    ...
    record = ShallowDiffuseOfficialReferenceRecord(
        reference_record_id=f"shallow_diffuse_official_reference_{digest[:16]}",
...
def validate_shallow_diffuse_official_reference_records(rows: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row_index, row in enumerate(materialized_rows):
        row_issues: list[ShallowDiffuseOfficialReferenceIssue] = []
        if _str_field(row, "baseline_id") != "shallow_diffuse":
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "baseline_id", "shallow_diffuse_baseline_required"))
        if _str_field(row, "reference_protocol_name") != SHALLOW_DIFFUSE_OFFICIAL_REFERENCE_PROTOCOL_NAME:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "reference_protocol_name", "official_reference_protocol_required"))
        ...
        for field_name in REQUIRED_SOURCE_PROVENANCE_FIELDS:
            if not _str_field(row, field_name):
                row_issues.append(
                    ShallowDiffuseOfficialReferenceIssue(
                        row_index,
                        field_name,
        ...
        if not _str_field(row, "baseline_result_source_digest"):
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "baseline_result_source_digest", "result_source_digest_required"))
        if not row.get("evidence_paths"):
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "evidence_paths", "evidence_paths_required"))
        if _str_field(row, "prompt_dataset_repository_id") != _PROMPT_DATASET_SOURCE.repository_id:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "prompt_dataset_repository_id", "registered_prompt_dataset_required"))
        if _str_field(row, "prompt_dataset_revision") != _PROMPT_DATASET_SOURCE.revision:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "prompt_dataset_revision", "registered_prompt_dataset_revision_required"))
        if _str_field(row, "official_model_repository_id") != _OFFICIAL_MODEL_SOURCE.repository_id:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "official_model_repository_id", "registered_official_model_required"))
        if _str_field(row, "official_model_revision") != _OFFICIAL_MODEL_SOURCE.revision:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "official_model_revision", "registered_official_model_revision_required"))
        if _str_field(row, "openclip_model_name") != OPENCLIP_MODEL_NAME:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "openclip_model_name", "registered_openclip_model_required"))
        if _str_field(row, "openclip_repository_id") != _OPENCLIP_SOURCE.repository_id:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "openclip_repository_id", "registered_openclip_repository_required"))
        if _str_field(row, "openclip_revision") != _OPENCLIP_SOURCE.revision:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "openclip_revision", "registered_openclip_revision_required"))
        if _str_field(row, "openclip_checkpoint_filename") != _OPENCLIP_REQUIRED_FILE.path:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "openclip_checkpoint_filename", "registered_openclip_checkpoint_required"))
        if _str_field(row, "openclip_checkpoint_sha256") != _OPENCLIP_REQUIRED_FILE.sha256:
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "openclip_checkpoint_sha256", "registered_openclip_checkpoint_sha256_required"))
        try:
            checkpoint_size = _int_field(row, "openclip_checkpoint_size_bytes")
        except (TypeError, ValueError):
            row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, "openclip_checkpoint_size_bytes", "registered_openclip_checkpoint_size_required"))
        else:
            ...
            try:
                ...
                row_issues.append(
                    ShallowDiffuseOfficialReferenceIssue(
                        row_index,
                        field_name,
                        "metric_value_must_be_finite_numeric",
                    )
                )
                continue
            if field_name.endswith("count") and value <= 0:
                row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, field_name, "positive_count_value_required"))
            if field_name in {"auc", "accuracy", "true_positive_rate_at_one_percent_fpr"}:
                if not 0.0 <= value <= 1.0:
                    row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, field_name, "metric_rate_must_be_in_unit_interval"))
            if field_name in {"clip_score_mean", "watermarked_clip_score_mean"}:
                if not -1.0 <= value <= 1.0:
                    row_issues.append(ShallowDiffuseOfficialReferenceIssue(row_index, field_name, "clip_cosine_similarity_must_be_in_signed_unit_interval"))
        if row_issues:
