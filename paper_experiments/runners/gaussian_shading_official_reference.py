...
class GaussianShadingOfficialReferenceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.run_official_command and not self.prepare_legacy_environment and not self.official_python_executable:
...
def write_gaussian_shading_official_reference_outputs(
    ...
    with progress_bar(10, desc="gaussian shading official reference", enabled=config.enable_workflow_progress_bar) as run_progress:
        ...
        environment_config = config if config.run_official_command else replace(config, prepare_legacy_environment=False)
...
def package_gaussian_shading_official_reference_outputs(
    ...
    archive_name: str = "external_baseline_official_reference_package_gaussian_shading.zip",
