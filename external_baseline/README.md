...
三套 official-reference runner 分别使用 `tree_ring_official_py39_cu117`、`gaussian_shading_official_py38_cu117` 与 `shallow_diffuse_official_py39_cu117` 隔离子解释器。五个 CUDA profile 的锁文件、解释器、wheel 和设备身份都必须在匹配的 Linux x86_64 CUDA 环境中复验；仓库中存在锁文件不等于某次 GPU 科学运行已经完成。
...
SLM-WM 的目标 HF-tail 载体必须先对密钥高斯模板执行二维高通，再按冻结 tail 规则选择坐标；其唯一算法定义见 `../docs/builds/algorithm_primitives_content_adaptive_dual_carrier_latent_watermark.md` 第7节。本 README 只划分 baseline 职责，不重复冻结比例、排序或权重。Gaussian Shading 是独立外部方法，具有自己的消息编码、latent 分布构造和 voting 检测协议；不得因二者都涉及高斯采样而合并概念。
