...
三套 official-reference runner 分别使用 `tree_ring_official_py39_cu117`、`gaussian_shading_official_py38_cu117` 与 `shallow_diffuse_official_py39_cu117` 隔离子解释器。五个 CUDA profile 的完整哈希锁已经写入 `configs/dependency_profiles/` 并受 registry 与 harness 校验。真实运行仍必须在匹配的 Linux x86_64 CUDA 环境中复验锁、解释器、wheel 和设备身份; 仓库存在锁文件不等于某次 GPU 科学运行已经完成。
