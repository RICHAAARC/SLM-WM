...
4. 在本地 Windows / CPU 汇总环境通过 `scripts/run_gpu_server_result_closure.py` 调用 `scripts/paper_result_closure.py`, 冻结精确10包输入并执行18步结果记录、共同协议、统计分析、证据门禁和完整结果包重建。主表结论必须同时通过全样本配对优势与检测标签无关的逐 Prompt SSIM 质量匹配优势门禁。
