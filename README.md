...
3. 通过各自 Notebook 或 `scripts/run_gpu_server_workflow.py` 的对应路由运行三个 common-backbone method-faithful、T2SMark 和三个 official-reference 原环境复现。official-reference 结果只进入补充方法忠实度审计。
4. 在 CPU 汇总环境通过 `scripts/run_gpu_server_result_closure.py` 调用 `scripts/paper_result_closure.py`, 冻结精确10包输入并执行18步结果记录、共同协议、统计分析、证据门禁和完整结果包重建。
