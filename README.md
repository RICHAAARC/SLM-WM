...
正式公平比较采用3个生成种子与3个水印密钥组成的9个交叉重复。每个 Prompt 在同一重复中为 SLM-WM、Tree-Ring、Gaussian Shading、Shallow Diffuse 和 T2SMark 分配相同的公开生成 seed, 并由 `sha256_counter_normal_icdf_table20_float32_v2` 构造同一个基础 latent Tensor。该协议从 SHA-256 大端计数器块组成的 MSB-first 连续比特流提取20位索引, 查询 $q_i=\operatorname{round}_{\mathrm{binary32}}\!\left(\Phi^{-1}((i+0.5)/2^{20})\right)$ 的冻结 Q20 表；表的完整大端字节 SHA-256 为 `70abf440a7f3670147965ffa52f5aaa639dab97f6282b68f3a9a1b1ce5e6cf5a`。输出是在 CPU 上规范生成的有限离散 Q20 量化标准正态, 不是连续精确的 $\mathcal N(0,1)$；目标 dtype 转换也在 CPU 完成, 随后才搬运到执行设备。该 Tensor 以实际字节摘要绑定到全部方法记录；任一方法的 seed、密钥重复索引、基础 latent 内容摘要或身份摘要不同, 该样本不得进入配对统计。各水印方法继续使用自身编码和检测算子, 共同协议只控制生成随机性、重复身份和公平预算。`SLM_WM_RANDOMIZATION_REPEAT_ID` 每次只选择一个登记重复, Drive 输出按重复隔离；论文正式汇总必须覆盖全部9个重复, 单个重复不能替代多种子、多密钥证据。

Q20 中点离散化的理想 KS 距离为 $2^{-21}$, 含 float32 舍入的登记上界为 `4.912236096776823e-7`。53位开区间 uniform 路径与 Q20 路径分离, 只用于注意力关系符号。MPFR 逐项舍入复验是外层审计证据, 不进入 PRG 算法摘要或采样身份。当前逐字节固定向量只在 Windows CPU 实测；Linux/Colab 一致性必须由 GPU 运行前门禁复验, 不能把算法定义的设备无关性写成已经完成跨平台实测。
...
项目使用同一批 repository modules、服务器命令和 Notebook 入口支撑三类论文运行层级。三类运行层级在方法参数、攻击协议、baseline 入口、FPR=0.1 工作点、Wilson 单侧 FPR 上界、Hoeffding 结果区间、随机种子、证据门禁和结果闭合逻辑上必须保持一致; 运行语义只允许 Prompt 数量与统计强度不同。
...
| `pilot_paper` | 700 | 0.1 | `pilot_claim` | `/content/drive/MyDrive/SLM/pilot_paper_results` | dev/calibration/test 为 30/330/340。 |
| `full_paper` | 7000 | 0.1 | `full_claim` | `/content/drive/MyDrive/SLM/full_paper_results` | dev/calibration/test 为 300/3300/3400。 |
