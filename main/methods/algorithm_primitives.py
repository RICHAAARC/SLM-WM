...
项目特定写法: LF、幅值尾部截断分支与 attention synthetic stub 均从同一
语义安全子空间导出。几何链只允许辅助 rescue, attestation 只影响 final-level。
"""
...
def _quantile(values: Sequence[float], quantile: float) -> float:
    """计算简单确定性分位数, 用于 synthetic 高斯幅值尾部截断。"""
    if not values:
...
class SemanticRiskField:
    """语义风险场与 LF/尾部截断/attention 路由 mask。"""

    risk_values: tuple[float, ...]
    budget_values: tuple[float, ...]
    lf_mask: tuple[bool, ...]
    tail_mask: tuple[bool, ...]
...
class SafeBasisEstimate:
    ...
    def __post_init__(self) -> None:
        ...
        rows = [*self.safe_basis, *self.lf_basis, *self.tail_basis, *self.attention_basis]
        if rows:
...
class CarrierPrimitive:
    """LF、尾部截断或 attention synthetic stub 载体。

    `frequency_band` 对尾部截断分支固定为不适用, 因为该分支没有空间频带定义。
    """
        ...
        _ensure_equal_lengths(
            {
                ...
                "tail_update_values": self.tail_update_values,
...
def build_semantic_risk_field(
    ...
    for index in range(length):
        ...
        tail_mask.append(texture_vector[index] >= config.texture_threshold and stability_vector[index] >= config.stability_threshold)
...
def estimate_safe_basis(
    ...
    tail_basis = _one_hot_basis(length, route_indices(risk_field.tail_mask), risk_field.budget_values)
...
def derive_tail_carrier(
    ...
    """从 `tail_basis` 导出幅值尾部截断补充载体。"""
    template = _stable_unit_values(
        (key, event_digest, safe_basis.basis_digest, safe_basis.route_digest, "tail_robust"),
        len(safe_basis.tail_basis),
    )
    weights = []
    for row in safe_basis.tail_basis:
        active_index = max(range(len(row)), key=lambda index: abs(row[index]))
        weights.append(risk_field.budget_values[active_index] + (1.0 if risk_field.tail_mask[active_index] else 0.0))
    ...
    if retained_count == 0:
        ...
        retained_count = 1
    update = _basis_weighted_update(safe_basis.tail_basis, truncated_template, embedding_strength)
    carrier_payload = {
        "carrier_family": "gaussian_tail_robust",
        "frequency_band": "not_applicable_amplitude_tail",
        "template_values": [round(value, 12) for value in truncated_template],
        "update_values": [round(value, 12) for value in update],
        "tail_threshold": round(threshold, 12),
    }
    return CarrierPrimitive(
        carrier_id=f"tail_{build_stable_digest(carrier_payload)[:12]}",
        carrier_family="gaussian_tail_robust",
        frequency_band="not_applicable_amplitude_tail",
        key_digest=build_stable_digest({"key": key, "event_digest": event_digest, "branch": "tail_robust"}),
...
def derive_attention_carrier_stub(
    ...
    它只提供与 LF/尾部截断分支同源的几何 update 形状, 便于后续运行单元替换为真实运行时实现。
    """
    template = _stable_unit_values(
...
def compose_latent_update(
    ...
    """组合三个载体分量, 返回 `Delta z_t = LF + tail + A`。"""
    length = _ensure_equal_lengths(
        {
            "lf_update_values": lf_carrier.update_values,
            "tail_update_values": tail_carrier.update_values,
            "attention_update_values": attention_carrier.update_values,
        }
    )
    combined = tuple(
        lf_carrier.update_values[index] + tail_carrier.update_values[index] + attention_carrier.update_values[index]
        for index in range(length)
    )
...
def compute_content_score(
    ...
    tail_raw_score = _correlation(observed_vector, tail_carrier.update_values)
    tail_score = tail_raw_score * tail_carrier.retained_fraction
    content_score = lambda_lf * lf_score + lambda_tail * tail_score
